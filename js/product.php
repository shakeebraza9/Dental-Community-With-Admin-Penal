...
function select_change_payment() {
    ...
    if ( $('.paymentOptionRadio').is(":checked") ) {
        ...
        if (paymentOption == '0' || paymentOption == '1' || paymentOption == '5' || paymentOption == '8' || paymentOption == '9' ) {
            $("#cartLoading").slideDown(500);
            inv = $('#invoiceId').val();
            $.ajax({
                type: "POST",
...
function quickView(ths,productId){
    ...
    $("#productQuickView").html(loading_progress()).load(url);
    $("#close_side").show();
...
<?php ### check in developer setting is cart_load_from_side_enabled ?>
        var cart_load_from_side_enabled = <?php echo ($functions->developer_setting('cart_checkout_from_side_modal')); ?> 0;

        if ( cart_load_from_side_enabled != 1 ) {
            $("#cartLoading").slideDown(500);
        } else {
            $("#cartLoading").slideDown(500);
              $('.background_side').show();
            $('.overlay').show();
        ...
        $.ajax({
            type: "POST",
            url: url,
            data: {pId: pId, storeID: storeId, scaleId: scaleId, colorId: colorId , customQty:customQty}
        }).done(function (data){
            if(data == '1' || data == ''){
...
function updateCartNoOnWeb() {
    ...
    no = eval(no + 1);
    $.ajax({
        url: 'cartWebUpdate.php',
        type: 'POST',
        data: {'qtyy':no,'cat_item':'true'},
     }).done(function(res){
        // console.log('Ajax Response check : '+res);
        if(res != '0'){
           var noOfItems_res = $(res).filter('#noOfItems');
           var cartPrice_res = $(res).filter('#cartPrice');
           var cur_symbol_res = $(res).filter('#cur_symbol');

           var noOfItems = noOfItems_res.val();
           var cartPrice = cartPrice_res.val();
           var cur_symbol = cur_symbol_res.val();

           $(".cartItemNo").text(noOfItems);
           $(".cartPriceAjax").text(cartPrice+cur_symbol);
           //console.log('noOfItems: '+noOfItems+' cartPrice_res: '+cartPrice+' cur_symbol: '+cur_symbol); 
        }
...
*/
    ...
    if(allOk == false){
        jAlertifyAlert("Please select product's size/color of all package products");
        return false;
    }else {
       // pArry = "["+pArry+"]";
        $("#packInfo").val(pArry);
...
function add_payment_method_price(){
    ...
    else if(price > 0 && payment_type == 8){
        total_price = parseFloat($(".pGrandTotal").attr("data-total"));
        total_price2 = total_price+((total_price)*(price/100));
        $(".pGrandTotal").text(total_price2);

        //add in shipping price
        shipping_price = parseFloat($('.pShippingPriceTemp').attr("data-real"));
        shipping_price = shipping_price+((total_price)*(price/100));
        $('.pShippingPriceTemp').text(shipping_price);
    }else{
        total_price = parseFloat($(".pGrandTotal").attr("data-total"));
        $(".pGrandTotal").text(total_price);
...
function total_text_for_input() {
    var total_text_for_input = "<?php $dbF->hardWords("- ORDER - ") ?> ";
