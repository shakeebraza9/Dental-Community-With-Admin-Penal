...
<div class="index_content mypage">
    <div class="left_right_side">
        ...
        <h4 class="c4">Mandatory Over Due Compliance Activities.</h4>
        <h4 class="c4">Below are your overdue mandatory activities which can be completed on this page or through your activity calender.</h4>
 <div data-toggle="tooltip" title="Help Video" class="help" onclick="video('EJUtPIhK-Bg')"><i class="fa fa-question-circle"></i></div>
                ...
                <?php if($_SESSION['currentUserType'] !='Employee' || @$_SESSION['superUser']['ccalendar'] == 'edit' || @$_SESSION['superUser']['ccalendar'] == 'full') { 
                    $sql = "SELECT * FROM `userevent` WHERE `due_date` = CURRENT_DATE() AND `due_date` != '' AND `approved`='-1' AND `user`='$_SESSION[currentUser]' AND `title_id` IN (SELECT `id` FROM `eventmanagement` WHERE `recurring_duration` IN ('1 day','1 week')) AND `userevent`.`title_id` NOT IN (SELECT files_id FROM `practiceFileSelection` WHERE user_id='$user' AND type='event')";
        ...
        <li><a href="#tabs-4">All Mandatory OverDue Event  </a></li>                       
