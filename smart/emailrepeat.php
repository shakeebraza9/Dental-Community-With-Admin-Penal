...
$date = Date('Y-m-d');
$q = "SELECT `id_user`,`setting_val` FROM `accounts_user_detail` 
        WHERE `setting_val` IN ('Master','Practice') 
        AND `id_user` IN (SELECT `acc_id` FROM `accounts_user` WHERE `notify` = '1') 
        AND `id_user` NOT IN(SELECT `acc_id` FROM `accounts_user`WHERE acc_type=0) 
        AND `id_user` NOT IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_val='Trial') 
        AND `id_user` NOT IN (SELECT `id_user` FROM `accounts_user_detail` 
                        WHERE setting_name='account_under' 
                        AND (setting_val IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_val='Trial') 
                        OR setting_val IN (SELECT acc_id FROM `accounts_user` WHERE acc_type=0)
                        )
                    )";
$datas = $dbF->getRows($q);
...
// $sql = "SELECT * FROM `accounts_user` WHERE acc_id='2' #AND `weekly_email` < '$chkDate'";      AND `notify` = '1' 
$sql = "SELECT * FROM `accounts_user` 
            WHERE `weekly_email` < DATE_ADD(CURDATE(), INTERVAL -6 DAY) 
            AND acc_type=1 
            AND `acc_id` NOT IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_val='Trial') 
            AND `acc_id` NOT IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_name='account_under' 
                                    AND (setting_val IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_val='Trial') 
                                    OR setting_val IN (SELECT acc_id FROM `accounts_user` WHERE acc_type=0)
                                        )
                                ) LIMIT 0, 800";
...
$temp = "<table>";
$s_no = 1;
$temp .= "<tr><th>Sno</th><th>User</th><th>Subject</th></tr>";
    foreach ($data as $key => $value) {
    
    $id = intval($value['acc_id']);
    $name = $value['acc_name'];
    
    $row1 = "SELECT setting_val from accounts_user_detail where setting_name = 'account_type' AND id_user = '$id' ";
    $data1 = $dbF->getRow($row1);
    
    $row2 = "SELECT setting_val from accounts_user_detail where setting_name = 'account_under' AND id_user = '$id' ";
    $data2 = $dbF->getRow($row2);
    
    $acc_type = $data1['setting_val'];
    $pid = $data2['setting_val'];
    
    
    // $dueEvent = $functions->dueEvent($id);
    // $overdueEvent = $functions->overdueEvent($id);
    // $newDocuments = $functions->newDocuments($id);
    $dueEvent = $functions->dueEvent($id,$acc_type,$pid);
    $overdueEvent = $functions->overdueEvent($id,$acc_type,$pid);
    $notSubmitStaffDoc = $functions->notSubmitStaffDoc("staffDocNotSubmit",$id,$name,$email);
    
    $content = "<b>Overdue Events</b>$overdueEvent<b>Due Events</b>$dueEvent";
    ...
    $sqlInsert = "INSERT into `email_record` (`user`,`type`,`subject`,`email`,`mail`,`player_id`) VALUES (?,?,?,?,?,?)";
    $arrayInsert   = array($id, 'weeklyEmail',$subject,$email, $content, $functions->getUserPlayerId($id));
    $dbF->setRow($sqlInsert, $arrayInsert);
    
    if($value['notify']==1){
        $sqlInsert = "INSERT into `email_queue` (`email_to`,`subject`,`message`) VALUES (?,?,?)";
        $dbF->setRow($sqlInsert, array($email,$subject,$content));
    }
    
    // $functions->send_mail($email,$row['subject'],$content);
    
    $temp .= "<tr><td>".$s_no."</td><td>".$email."</td><td>".$row['subject']."</td></tr>";
    $s_no++;
...
$functions->send_mail('mobashir@im.com.pk','Staff Doc And Event Email',$temp);
/* Weekly email */

$sql = "SELECT `id_user`,`setting_val` FROM `accounts_user_detail` 
            WHERE `setting_val` IN ('Master','Practice') 
            AND `id_user` IN (SELECT `acc_id` FROM `accounts_user` WHERE `notify` = '1') 
            AND `id_user` NOT IN (SELECT `acc_id` FROM `accounts_user`WHERE acc_type=0) 
            AND `id_user` NOT IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_val='Trial') 
            AND `id_user` NOT IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_name='account_under' 
                                    AND (setting_val IN (SELECT `id_user` FROM `accounts_user_detail` WHERE setting_val='Trial') 
                                    OR setting_val IN (SELECT acc_id FROM `accounts_user` WHERE acc_type=0)
                                    )
                                )";
...
$data= $functions->dbF->getRows("SELECT `acc_id` FROM `accounts_user` WHERE last_login NOT BETWEEN DATE_ADD(CURDATE(), INTERVAL -31 DAY) 
