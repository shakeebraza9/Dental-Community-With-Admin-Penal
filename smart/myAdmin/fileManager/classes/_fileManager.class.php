...
class filesManager extends object_class{
    ...
    public function newFilesManagerAdd(){
        ...
        if(isset($_POST['submit'])){
            ...
            $heading        = empty($_POST['file_heading'])   ? ""    : ($_POST['file_heading']);
...
// $heading = $heading[$i];
        ...
        $sql      =   "INSERT INTO `filesmanager`(
                                    `link`, `file_heading`,`layer0`,`publish`)
                                    VALUES (?,?,?,?)";

                // $array   = array($link,$heading[0]['English'],$layer0,$publish);
                // $this->dbF->setRow($sql,$array,false);
                // $lastId = $this->dbF->rowLastId;

            

                $array   = array($link,$heading,$layer0,$publish);
                $this->dbF->setRow($sql,$array,false);
                $lastId = $this->dbF->rowLastId;
...
return $opt;
    ...
    public function filesManagerEdit($new=false){
        ...
        @$file_heading = $data['file_heading'];
        ...
        for ($i = 0; $i < sizeof($lang); $i++) {
            ...
            // }else{ echo '<input type="hidden" name="file_heading['.$lang[$i].']" value="" class="form-control">';}
                     ...
                     $lay1_Status = '<div class="input-group">
                      ...
                      <input type="text" class="form-control"                                  name="file_heading[]['.$lang[$i].']" id="user"
                               placeholder="'.$_e['Type File Title Here'].'" value="" required">





                                <input type="text"  name="layer1[]['.$lang[$i].']" value="" class="layer1'.$i.' form-control" placeholder="'.$_e['Select File'].'">
                                <div class="input-group-addon pointer " onclick="'."openKCFinderFile($('.layer1".$i."'))".'"><i class="glyphicon glyphicon-file"></i></div>
                            </div>';

        echo '<div class="form-group">

                        <label class="col-sm-2 col-md-3  control-label">'. $_e['Files'] .'</label>

                        <div class="col-sm-10  col-md-9 append_video">

                            <div class="input-group form-group">

                                <div class="input-group-addon pointer "><input type="checkbox" name="video_check" disabled></div>
                                     <div class="col-sm-10  col-md-9">
                        '.$lay1_Status.'
                        </div>
        ...
        echo '<div class="form-group">
                    ...
                    <a class="btn btn-primary" onclick="add_video()" >Add More</a>

                    <a class="btn btn-danger" onclick="del_video()" >Delete</a>
    ...
    var add_field = '<div class="input-group form-group"><div class="input-group-addon pointer "><input type="checkbox" name="video_check"></div><input type="text" class="form-control" name="file_heading[][<?php echo $lang[$i] ?>]" id="user" placeholder="<?php echo $_e['Type File Title Here'] ?>" value="" required"><input type="text"  name="layer1[][<?php echo $lang[$i] ?>]" value="" class="layer1 form-control" placeholder="<?php echo $_e['Select File'] ?>"><div class="input-group-addon pointer " onclick='+"openKCFinderFile($('.layer1'))"+'><i class="glyphicon glyphicon-file"></i></div></div>';

    $('.append_video').append(add_field);

}

function del_video(){

   $('input[name=video_check]:checked').parent().parent().remove();
