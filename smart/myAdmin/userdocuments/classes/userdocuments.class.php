...
class userdocuments extends object_class{
    public $productF;
    public function __construct(){
        ...
        $_w['Manage User Documents'] = '' ;

        //filesManager.php
        $_w['Active User Documents'] = '' ;
        $_w['Draft'] = '' ;
        $_w['Add New User Documents'] = '' ;
        $_w['Delete Fail Please Try Again.'] = '' ;
        $_w['There is an error, Please Refresh Page and Try Again'] = '' ;
        $_w['SNO'] = '' ;
        $_w['User Documents Title'] = '' ;
        $_w['IMAGE'] = '' ;
        $_w['ACTION'] = '' ;
        $_w['Image User Documents Error'] = '' ;
        $_w['Image Not Found'] = '' ;
        $_w['User Documents'] = '' ;
        $_w['Added'] = '' ;
        $_w['User Documents Add Successfully'] = '' ;
        $_w['User Documents Add Failed'] = '' ;
        $_w['User Documents Update Failed'] = '' ;
        $_w['User Documents Update Successfully'] = '' ;
        $_w['Update'] = '' ;
        $_w['Title'] = '' ;
        $_w['File'] = '' ;
        $_w['Short Desc'] = '' ;
    ...
    public function userdocumentsView(){
        $sql  = "SELECT * FROM userdocuments ORDER BY ID DESC";
        $data =  $this->dbF->getRows($sql);
        $this->userdocumentsPrint($data);
    }

    // public function userdocumentsDraft(){
    //     $sql  = "SELECT * FROM userdocuments WHERE publish='0' ORDER BY ID DESC";
    //     $data =  $this->dbF->getRows($sql);
    //     $this->userdocumentsPrint($data);
    // }

    public function userdocumentsPrint($data){
        global $_e;
         $uniq=uniqid('id');
        $href = 'userdocuments/userdocuments_ajax.php?page=active_userdocuments';
      ...
      echo  '
            ...
            <table class="table table-hover tableIBMS dTable_ajax " data-href="'.$href.'" data-uniq="'.$uniq.'">
                    <thead>
                        ...
                        <th>'. _u($_e['File']) .'</th>
                        <th>'. _u('Completion Date') .'</th>
                        <th>'. _u('Expiry Date') .'</th>
                        <th>'. _u('Description') .'</th>
                        <th>'. _u($_e['ACTION']) .'</th>
                    </thead>
                <tbody>';
        ...
        //                     <!--<a data-id='$id' href='-userdocuments?page=edit&userdocumentsId=$id' class='btn'>
    ...
    public function newuserdocumentsAdd(){
        global $_e;
        if(isset($_POST['submit'])){
            if(!$this->functions->getFormToken('newuserdocuments')){return false;}
...
htmlspecialchars($desc);
            ...
            try{
                ...
                if($this->dbF->rowCount>0){
                    $this->functions->notificationError(_uc($_e['userdocuments/Document']),($_e['Training/Document Add Successfully']),'btn-success');
    ...
    public function userdocumentsEditSubmit(){
        global $_e;
        if(isset($_POST['submit'])){
...
htmlspecialchars($desc);
    ...
    public function userdocumentsNew(){
        global $_e;
        $this->userdocumentsEdit(true);
    }
...
return $opt;
    ...
    public function userdocumentsEdit($new=false){
        global $_e;
        if($new){
