...
class ajax extends object_class{
    ...
    public function  __construct()
    {
        ...
        if (isset($GLOBALS['productF'])) $this->product = $GLOBALS['productF'];
            ...
            require_once(__DIR__."/../../product_spb/classes/product.class.php");
    ...
    public function AjaxAfterUpdateScript_color(){
        $id=$_GET['id'];
        $sql_2 = "SELECT * FROM `colors` WHERE `color_name_id` = ? ";
        $data=$this->dbF->getRows($sql_2,array($id));
    ...
    public function AjaxAfterUpdateScript_scale(){
        $id=$_GET['id'];
        $sql_2 = "SELECT * FROM `scales` WHERE `scale_name_id` = ? ";
        $data=$this->dbF->getRows($sql_2,array($id));
    ...
    public function AjaxAfterUpdateScript_currency(){
        $id=$_GET['id'];
        $data = $this->dbF->getRow("SELECT * FROM `currency` WHERE `cur_id`= ?",array($id));
        if($this->dbF->rowCount > 0){
    ...
    public function AjaxDelScript_product()
    {
        try{
            ...
            $sql3="SELECT * FROM `product_image_spb` WHERE `product_id`= ? ";
    ...
    public function AjaxDelScript_productImageDel(){
        ...
        $sql3="SELECT * FROM `product_image_spb` WHERE `img_id`= ? ";
        $data=$this->dbF->getRow($sql3,array($id));

        unlink(__DIR__."/../../../images/$data[image]");

        $sql3="DELETE FROM `product_image_spb` WHERE `img_id`='$id'";
        $this->dbF->setRow($sql3);
    ...
    function AjaxDelScript_storeDel(){
        ...
        $sql="SELECT * FROM  `product_inventory`  WHERE `qty_store_id`= ?  AND `qty_item`>'0'";
        $this->dbF->getRows($sql,array($id));
        if($this->dbF->rowCount>0){
    ...
    public function AjaxEditStore(){
        global $_e;
        $id = $_GET['id'];
        $sql="SELECT * FROM `store_name` WHERE `store_pk` = ? ";
    ...
    public function AjaxAfterUpdateScript_store(){
        $id = $_GET['id'];
        $sql="SELECT * FROM `store_name` WHERE `store_pk` = ? ";
        $val = $this->dbF->getRow($sql,array($id));
        echo "
                    <td>*</td>
    ...
    public function AjaxDelScript_couponDel(){
        $id     =   $_POST['id'];
        $sql ="DELETE FROM `product_coupon_spb` WHERE pCoupon_pk = '$id'";
        $this->dbF->setRow($sql);
        if($this->dbF->rowCount>0){
            echo "1";
        }else{
    ...
    public  function sortProductImage(){
        ...
        for ($i = 0; $i < count($list); $i++) {
            $sql3="UPDATE `product_image_spb` SET sort='$i' WHERE `img_id`='$list[$i]'";
    ...
    public  function pImageAltUpdate(){
        $id=$_POST['imageId'];
        $alt=$_POST['altT'];
        $sql3="UPDATE `product_image_spb` SET alt=? WHERE `img_id`='$id'";
        $array = array($alt);
    ...
    public  function sortProducts(){
        $list=$_POST['sort'];
        for ($i = 0; $i < count($list); $i++) {
            $sql3="UPDATE `proudct_detail_spb` SET sort='$i' WHERE `prodet_id`='$list[$i]'";
            $data=$this->dbF->setRow($sql3);
        }
    }
    ...
    public  function featureItem(){
        global $_e;
            ...
            $sql2   =   "UPDATE proudct_detail_spb set feature = '$val' WHERE prodet_id = '$id'";
    ...
    public  function fetch_products(){
        ...
        if($search) { $search_sql = "
                                        ( `proudct_detail_spb`.`prodet_shortDesc` LIKE '%{$search}%'
                                            OR `proudct_detail_spb`.`prodet_name` LIKE '%{$search}%' )
                                        AND
                                    ";
        } else { $search_sql = ''; }

        ############# GET TOTAL ROWS #############
        $total_count_sql = " SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val` 
                FROM `proudct_detail_spb` 
                join `product_setting_spb`  on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
                WHERE 

                {$search_sql}

                `product_setting_spb`.`setting_name`='publicAccess' 
                AND `product_setting_spb`.`setting_val`={$setting_val} 
        ...
        if ($page == 'pending_products') {
            ...
            $date=date('m/d/Y');
            $total_count_sql = $qry="  SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
                    FROM `proudct_detail_spb` join `product_setting_spb`
        ...
        $qry = "SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
                ...
                AND `product_setting_spb`.`setting_val`={$setting_val} 
                AND `proudct_detail_spb`.`product_update`='1'
                ORDER BY `proudct_detail_spb`.`prodet_id` DESC LIMIT {$start},{$length} ";
        ...
        foreach($data as $key => $val){
            ...
            $category = translateFromSerialize($val['category']);
