...
class product_function extends object_class{
    ...
    public function productSaleCountPlus($pId,$qty){
            $sql = "UPDATE proudct_detail_spb SET `sale` = `sale`+$qty WHERE prodet_id = '$pId'";
            $this->dbF->setRow($sql);
    }
    ...
    public function getProductWeight($pId,$scaleId=0,$colorId=0){
        ...
        $sql = "SELECT * FROM `product_setting_spb` WHERE p_id = '$pId' AND setting_name = 'defaultWeight'";
    ...
    public function getProductName($pid){
        ...
        $sql    =   "SELECT `prodet_name` FROM `proudct_detail_spb` WHERE `prodet_id` = ? AND product_update = ?";
        $data   =   $this->dbF->getRow($sql,array($pid,'1'));
        if($this->dbF->rowCount>0){
    ...
    public function getProductSetting($pId){
        $qry    =   "SELECT * FROM  `product_setting_spb` WHERE `p_id` = ? ";
    ...
    public function getStoreName($id){
        ...
        $sql="SELECT `store_name`,`store_location` FROM `store_name` WHERE `store_pk` = ? ";
        $data = $this->dbF->getRow($sql,array($id));
    ...
    public function customSizePrice($pId,$currency=false){
        $currencyId = $currency;
        $sql        = "SELECT * FROM product_size_custom WHERE `pId` = ? AND currencyId = ? ";
        $customData = $this->dbF->getRow($sql,array($pId,$currencyId));
...
// var_dump($countryPK);
    ...
    public function productPrice($productId,$currId=''){
        ...
        $sql ="SELECT * FROM `product_price_spb` WHERE  `propri_prodet_id` = ? AND `propri_cur_id` = ? ";
        $data=$this->dbF->getRow($sql,array($productId,$currId));
    ...
    public function scalePrice($scaleId,$currId,$pid){
        ...
        */
        $sql="SELECT `prosiz_name` FROM `product_size` WHERE `prosiz_id` = ? ";
        $temp=$this->dbF->getRow($sql,array($scaleId));
        $scaleName=$temp['prosiz_name'];
        $sql="";

        $sql ="SELECT `prosiz_id`,`prosiz_price`,`sizeGroup`
                FROM `product_size`
                WHERE  `prosiz_prodet_id` = ? AND `prosiz_name` = ? AND `prosiz_cur_id` = ? ";
        $data=$this->dbF->getRow($sql,array($pid,$scaleName,$currId));
        if($this->dbF->rowCount>0)return $data;
        else return 0;
    }
    ...
    public function colorPrice($colorId,$currId,$pid){
       ...
       */
        $sql="SELECT `proclr_name` FROM `product_color` WHERE `propri_id` = ? ";
        $temp=$this->dbF->getRow($sql,array($colorId));
        $colorName=$temp['proclr_name'];
        $sql="";

        $sql ="SELECT `propri_id`,`proclr_price`,`sizeGroup` FROM `product_color` WHERE `proclr_prodet_id` = ? AND `proclr_name` = ? AND `proclr_cur_id` = ? ";
        $data=$this->dbF->getRow($sql,array($pid,$colorName,$currId));
    ...
    public function currencyInfo($countryPKorId,$id=false){
        if($id==false) {
            ...
            $sql = "SELECT * FROM `currency` WHERE  `cur_id` = ? ";
        }

// echo $sql;

        $data=$this->dbF->getRow($sql,array($countryPKorId));
        if($this->dbF->rowCount>0)return $data;
        else return "";
    }

    public function currencySymbol($currencyId){
        $sql ="SELECT cur_symbol FROM `currency` WHERE  `cur_id` = ? ";
        $data=$this->dbF->getRow($sql,array($currencyId));
    ...
    public function stockProductQty($hash){
        $sql    =   "SELECT `qty_item` FROM `product_inventory` WHERE `product_store_hash` =  ? ";
        $inventoryData = $this->dbF->getRow($sql,array($hash));
    ...
    /*    public function AjaxEditScript($class,$name){
            ...
            id=$(this).attr('data-id');
                ...
                $.ajax({
                 type: 'POST',
                  url: 'product_management_spb/product_ajax.php?page=$class&id='+id
                }).done(function(data)
                {
                   if(data!='0'){
                     setTimeout(function(){
    ...
    public function AjaxUpdateScript($class,$name){
        ...
        echo "<script>
                ...
                $(ths).button('loading');
                 url= 'product_management_spb/product_ajax.php?page=$class';
                ...
                function AjaxAfterUpdateScript(id){
                    ...
                    $.ajax({
                     type: 'POST',
                      url: 'product_management_spb/product_ajax.php?page=AjaxAfterUpdateScript_".$name."&id='+id
                    }).done(function(data)
                    {
                       if(data!='0'){
                         setTimeout(function(){
    ...
    public function hasStock($pId){
        $sql    =   "SELECT qty_pk FROM `product_inventory` WHERE qty_product_id = ?  AND qty_item > ? ";
        $data   =   $this->dbF->getRow($sql,array($pId,'0'));
        if($this->dbF->rowCount>0)
            return true;
        return false;
    }
    ...
    public function productSQL($columnName='*',$AllWithNewBlank=true){
        ...
        $sql="SELECT ".$columnName." FROM `proudct_detail_spb` $all ORDER BY `sort` ASC";
    ...
    public function productCategory($pId,$echo = true){
        $sql  = "SELECT procat_cat_id FROM `product_category_spb` WHERE procat_prodet_id = ? ";
        $data = $this->dbF->getRows($sql,array($pId));
        if($this->dbF->rowCount>0){
            $category = "";
            foreach($data as $val){
    ...
    public function productCountryId($country){
        $sql ="SELECT * FROM `currency` WHERE  `cur_country` = ? ";
        $data=$this->dbF->getRow($sql,array($country));
...
//get sale offer
        ...
        if(!empty($data) && $data !=false && $this->productSaleData !=false){
            ...
            $sql = " SELECT pSale_setting_value FROM `product_sale_setting` WHERE pSale_id =  AND pSale_setting_name = ? ";
            $setting  =  $this->dbF->getRow($sql,array($saleId,'discountFormat'));
            if($this->dbF->rowCount>0){
                $return = array();
    ...
    public function productDiscountFirst($pId,$countryId){
        ...
        $today  = date('Y-m-d');
        $sql ="SELECT `product_discount_spb`.*, `product_discount_prices`.*  FROM
                  `product_discount_spb` join `product_discount_prices`
                  on `product_discount_spb`.`product_discount_pk` = `product_discount_prices`.`product_dis_id`
...
// var_dump($discount);
    ...
    public function productCouponStatus($couponCode){
        ...
        //get sale offer
        $sql = "SELECT product_coupon_spb.* , product_coupon_prices_spb.* FROM product_coupon_spb
    ...
    public function productCurrencysymbol($countryCode){
        $sql        = "SELECT `cur_symbol` FROM `currency` WHERE cur_country = ? ";
        $data = $this->dbF->getRow($sql,array($countryCode));
        return $data[0];
    ...
    public function userNoOfItemsInCart($userId){
        $sql    =   "SELECT sum(qty) as qty FROM cart WHERE userId = ? ";
    ...
    public function productLastImage($id){
        $sql ="SELECT * FROM `product_image_spb` WHERE product_id = ?  ORDER BY sort ASC ";
        $data = $this->dbF->getRow($sql,array($id));
        return $data['image'];
    }

    public function productSpecialImage($id,$alt,$defaultFirstImageShowForMain=true){
        $sql ="SELECT * FROM `product_image_spb` WHERE product_id = '$id' AND alt = '$alt' ORDER BY sort ASC ";
        $data = $this->dbF->getRow($sql);
        $imag = $data['image'];
        if($imag==''){
    ...
    public function productOnSaleTrigger($pId){
        ...
        //get letter id
        $sql    = "SELECT id FROM  email_letters WHERE `email_type` = ? ";
        $dataLetter   = $this->dbF->getRow($sql,array($saleTriggerLetter));
        if(empty($dataLetter)){
            return false;
        }
        $letterId = $dataLetter['id'];

        $sql    = "SELECT email FROM  product_subscribe WHERE `p_id` = ? AND `type` = ? ";
        $data   = $this->dbF->getRow($sql,array($pId,'sale'));
    ...
    public function product_in_stock_trigger($pId,$store_id,$scale_id,$color_id){
        ...
        //get letter id
        $sql    = "SELECT id FROM  email_letters WHERE `email_type` = ? ";
        $dataLetter   = $this->dbF->getRow($sql,array($stockTriggerLetter));
        if( empty($dataLetter) ){
            return false;
        }
        $letterId = $dataLetter['id'];
       ...
       $sql="SELECT $column
                FROM
                   `proudct_detail_spb` join `product_setting_spb`
                    ...
                    AND `product_setting_spb`.`setting_val`='$active'
                    AND `proudct_detail_spb`.`product_update`='1'
                    $where
                    ORDER BY `proudct_detail_spb`.`prodet_id` $orderBy $limit";
 ...
 public function set_coupon_useage_record($user,$orderId,$coupon_id,$coupon,$date) {
        //use this function where some extra info need to save in db...
        //first time use to save coupon info
        

        $sql = "INSERT INTO coupon_useage_record(`user`,`order_id`,`coupon_id`,`coupon`,`dateTime`) VALUES(?,?,?,?,?)";
        $this->dbF->setRow($sql, array($user,$orderId,$coupon_id,$coupon,$date));
    }
    ...
    public function getSubCatIds($parent)
    {
        ...
        $data = $this->dbF->getRows($sql,array($parent));
        $cat = array();
        if ($this->dbF->rowCount > 0) {
            //1 2 dept
            foreach ($data as $val) {
    ...
    public function product_category_spb($pId){
        $sql = "SELECT * FROM product_category_spb WHERE procat_prodet_id = ? ";
    ...
    public function check_product_in_3_for_2($pId){
        ...
        $pro_cat    = $this->product_category_spb($pId);
