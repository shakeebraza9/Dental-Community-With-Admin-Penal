...
public function allFormdataContent2(){
        ...
        $sql  = "SELECT * FROM boardingFormData GROUP BY type DESC ";
        $data =  $this->dbF->getRows($sql);
        $i = 0;
        // var_dump($data);
...
<tbody>';
  $i = 0;
        $sql  = "SELECT * FROM boardingFormData where type = '$type' ORDER BY id DESC ";
        $variable =  $this->dbF->getRows($sql);
foreach ($variable as $key => $value) {
     $id = $value['id'];
 $prev_comment = $value['comment'];
...
public function allFormdata2(){
        ...
        //             </thead>
        //         <tbody>';
        $sql  = "SELECT * FROM boardingFormData GROUP BY type ASC ";
        $data =  $this->dbF->getRows($sql);
        $i = 0;
        foreach($data as $val){
            $i++;
