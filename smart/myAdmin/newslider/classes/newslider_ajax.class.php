...
class newslider_ajax extends object_class{
...
public function deleteslid(){
    ...
    try{
        ...
        $sql3="SELECT image FROM newslider WHERE id='$id'";
        $data=$this->dbF->getRows($sql3,false);
        foreach($data as $key=>$val){
            @unlink(__DIR__."/../../../images/$val[image]");
        }

        $sql2="DELETE FROM newslider WHERE id='$id'";
    ...
    public function newsliderSort(){
        ...
        for ($i = 0; $i < count($list); $i++) {
            $sql3="UPDATE `newslider` SET sort='$i' WHERE `id`='$list[$i]'";
