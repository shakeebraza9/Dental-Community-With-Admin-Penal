...
public function firstInsert(){
    ...
    try{
        ...
        $sqlD="SELECT * FROM `proudct_detail_spb`  WHERE `product_update` ='0'
        ...
        // delete old products...
          ...
          foreach($dataDel as $key=>$val){
              $id=intval($val['prodet_id']);

              $sql3="SELECT * FROM `product_image_spb` WHERE `product_id`='$id'";
              $data=$this->dbF->getRows($sql3,false);
              foreach($data as $key=>$val){
                  $this->functions->deleteOldSingleImage($val['image']);
                  //unlink(__DIR__."/../../../images/$val[image]");
              }
              $sql3="DELETE FROM `product_image_spb` WHERE `product_id`='$id'";
              $this->dbF->setRow($sql3,false);

              $sql3="DELETE FROM `proudct_detail_spb` WHERE `prodet_id`='$id'";
        ...
        $sql        =   "INSERT INTO `proudct_detail_spb` (`product_update`) VALUES ('0')";
        $lastId     =   $this->dbF->setRow($sql);
        $this->pid  =   $lastId;
        //update slug id
        $sql        =   "UPDATE `proudct_detail_spb` set slug='$lastId' WHERE prodet_id =? ";
        $this->dbF->setRow($sql,array($lastId));
}
    ...
    private function editProductDelOld($id){
       $sql="DELETE  FROM  `product_color` WHERE  `proclr_prodet_id` = '$id'";
        ...
        $sql="DELETE FROM  `product_addcost_spb` WHERE  `proadc_prodet_id` = '$id'";
        $this->dbF->setRow($sql);
        $sql="";
        ...
        $sql="DELETE FROM  `product_setting_spb` WHERE  `p_id` = '$id'";
        $this->dbF->setRow($sql);
        $sql="";

        $sql="DELETE FROM  `product_price_spb` WHERE  `propri_prodet_id` = '$id'";
        $this->dbF->setRow($sql);
        $sql="";
        $sql="DELETE FROM  `product_category_spb` WHERE  `procat_prodet_id` = '$id'";
        $this->dbF->setRow($sql);
    ...
    private function getset_additionalCost()
    {
        if (isset($_POST[$this->prefix_addCostCheckBox]) && !empty($_POST[$this->prefix_addCostCheckBox])) {
            ...
            $sql = "INSERT " . " INTO `product_addcost_spb`
    ...
    private function getset_currency()
    {
        if (isset($_POST[$this->prefix_currencyArray]) && !empty($_POST[$this->prefix_currencyArray])) {
            ...
            if (is_array($_POST[$this->prefix_currencyArray])) {
                $sql = "INSERT " . "INTO `product_price_spb`
    ...
    private function getset_setting()
    {
        ...
        if (isset($_POST[$this->prefix_setting]) && !empty($_POST[$this->prefix_setting])) {
            ...
            $sql="INSERT INTO `product_setting_spb`(`p_id`,`setting_name`,`setting_val`) VALUES ";
    ...
    private function getset_proInformation($lang)
    {
        ...
        if (isset($_POST[$this->prefix_productBasicInformation]) && !empty($_POST[$this->prefix_productBasicInformation]) ) {
            ...
            $sql = "UPDATE `proudct_detail_spb` SET
                        `prodet_name`=?,
                        `slug` =?,
                        `prodet_shortDesc`=?,
            ...
            $arry= array( serialize($pinfo['name']),$slug, serialize($pinfo['sdesc']), $pinfo['expire'],$pinfo['category'], @$pinfo['bill_mode']);
            $this->dbF->setRow($sql,$arry);
...
/*///////Edited bu MUD/////////*/
            ...
            foreach ($languages as $language) {
                $keywords[$language] = $pinfo['name'][$language] . ', ' . $pinfo['sdesc'][$language];
            }
    ...
    private function getset_categoryList()
    {
        if (isset($_POST[$this->prefix_productCategory]) && !empty($_POST[$this->prefix_productCategory])) {
            ...
            $sql = "INSERT " . "INTO `product_category_spb` (`procat_cat_id`,`procat_prodet_id`) VALUES (?,?)";
            $pid = $this->pid;
    ...
    public function createPricingViewSystem()
    {
        ...
        // If product is edit mode
                $eId=$this->editPid;
                $qry="SELECT * FROM  `product_price_spb` WHERE `propri_prodet_id` = ?  AND `propri_cur_id` = ? ";
                $eData=$this->dbF->getRow($qry,array($eId,$currency_id));
    ...
    public function createListOfCustomSize()
    {
        ...
        if($isEdit){
            ...
            $dataOld = $this->dbF->getRows($sql,array($pId));
        }
...
HTML;
    ...
    private function scaleListFormateEdit() {
        ...
        if($this->prefix_editPro!=""){
            // If product is edit mode
            $eId=$this->editPid;
            $qry="SELECT * FROM  `product_size` WHERE `prosiz_prodet_id` = ? ";
            $eData=$this->dbF->getRows($qry,array($eId));
            return $eData;
        }else{
            return false;
        }
...
HTML;
    ...
    private function colorListFormateEdit() {
        ...
        if($this->prefix_editPro!=""){
            // If product is edit mode
            $eId=$this->editPid;
            $qry="SELECT * FROM  `product_color` WHERE `proclr_prodet_id` = ? ";
            $eData=$this->dbF->getRows($qry,array($eId));
            return $eData;
        }else{
            return false;
...
HTML;
    ...
    public function productView()
    {
       // $qry="SELECT * FROM `proudct_detail_spb` WHERE `product_update` = '1'";
        $qry="SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
                FROM
                   `proudct_detail_spb` join `product_setting_spb`
                    on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
                    WHERE `product_setting_spb`.`setting_name`='publicAccess' AND `product_setting_spb`.`setting_val`='1' AND `proudct_detail_spb`.`product_update`='1'
                    ORDER BY `proudct_detail_spb`.`prodet_id` DESC";
    ...
    public function productDraft()
    {
        $qry="SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
                FROM
                   `proudct_detail_spb` join `product_setting_spb`
                    on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
                    WHERE `product_setting_spb`.`setting_name`='publicAccess' AND `product_setting_spb`.`setting_val`='0' AND `proudct_detail_spb`.`product_update`='1'
                    ORDER BY `proudct_detail_spb`.`prodet_id` DESC";
       ...
       $qry="SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
                FROM
                   `proudct_detail_spb` join `product_setting_spb`
                    on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
                    WHERE
                    `product_setting_spb`.`setting_name`='launchDate'
                    AND `product_setting_spb`.`setting_val`>'$date'
    ...
    private function product_list_View($qry,$page = ''){
        ...
        $href = "product_spb/products_listing_ajax.php?page=active_products";

        if($page=='draft'){
            $href = "product_spb/products_listing_ajax.php?page=draft_products";

            $qry="SELECT `proudct_detail_spb`.* FROM `proudct_detail_spb` WHERE `proudct_detail_spb`.`prodet_id` NOT IN (SELECT distinct(p_id) FROM product_setting_spb) AND `proudct_detail_spb`.`product_update`='1'
                    ORDER BY `proudct_detail_spb`.`prodet_id` DESC";
            ...
            $href = "product_spb/products_listing_ajax.php?page=pending_products";
        }
...
HTML;
    ...
    public function productEditImages(){
        ...
        if($this->prefix_editPro!=""){
            // If product is edit mode
            $eId=$this->editPid;
            $qry="SELECT * FROM  `product_image_spb` WHERE `product_id` =  ?  ORDER BY sort ASC";
            $eData=$this->dbF->getRows($qry,array($eId));
            if($this->dbF->rowCount>0){
...
HTML;
    ...
    public function productSelectedNode(){
        $eId=$this->editPid;
        $qry="SELECT * FROM  `product_category_spb` WHERE `procat_prodet_id` = ?";
        $eData=$this->dbF->getRow($qry,array($eId));
        if($this->dbF->rowCount>0)
    ...
    public function priceAdditionChargesEdit(){
        ...
        $eId    =   $this->editPid;
        $sql    =   "SELECT * FROM  `product_addcost_spb` WHERE `proadc_prodet_id` = ?  AND proadc_name NOT IN ($notIn)  ";
        $eData  =   $this->dbF->getRows($sql,array($eId));
    ...
    public function priceCheckOutChargesEdit(){
        ...
        $eId    =   $this->editPid;
        $sql    =   "SELECT * FROM  `product_addcost_spb` WHERE `proadc_prodet_id` = ?  AND proadc_name IN ($additionalCost_in) ";
        $eData  =   $this->dbF->getRows($sql,array($eId));
    ...
    public function productLastImage($id){
        $sql ="SELECT * FROM `product_image_spb` WHERE product_id = '$id' ORDER BY sort ASC ";
