...
mscale.checkbox_name = "<?php
echo $product->prefix_scaleCheckBox;
?>";

mscale.input_name = "<?php
echo $product->prefix_scaleName;
?>";

mscale.input_addprice_name = "<?php
echo $product->prefix_scaleCost;
?>";
...
addCharges.checkbox_name = "<?php
echo $product->prefix_addCostCheckBox;
?>";

addCharges.input_name = "<?php
echo $product->prefix_addCostName;
?>";

addCharges.input_addprice_name = "<?php
echo $product->prefix_addCostCost;
?>";
...
mcolors.checkbox_name = "<?php
echo $product->prefix_colorCheckBox;
?>";

mcolors.input_name = "<?php
echo $product->prefix_colorName;
?>";

mcolors.input_addprice_name = "<?php
echo $product->prefix_colorCost;
?>";
...
echo _js($_e['Size Name']);
?></span>' +

'<input type="text" class="form-control" value="" name="sizeWeightName[' + uniqueId + ']" placeholder="<?php
echo _js($_e['Size Name']);
?>">' +
...
'<span class="input-group-addon"><?php
echo _js($_e['Weight In KG']);
?></span>' +

'<input type="text" class="form-control" value="" name="sizeWeight[' + uniqueId + ']" placeholder="<?php
echo _js($_e['Weight In KG']);
?>,e.g: 200,2,0.2">' +
...
<input type="hidden" name="edit_pro" value="<?php
    echo $product->pid;
?>"/>

<input type="hidden" name="edit_proSubmit" value="<?php
    echo $product->pid;
?>"/>
...
data-toggle="tab"><?php
echo _uc($_e['Basic Information']);
?></a></li>

<!--<li><a href="#tab_categroy" data-toggle="tab"><?php //echo _uc($_e['Category']); 
?></a></li>-->

<li><a href="#tab_price" data-toggle="tab"><?php
echo _uc($_e['Price']);
?></a></li>
...
<li><a href="#tab_sizes" data-toggle="tab"><?php
    echo _uc($_e['Sizes']);
?></a></li>
...
<li><a href="#tab_colors" data-toggle="tab"><?php
    echo _uc($_e['Colors']);
?></a></li>
...
<?php
if ($functions->developer_setting("product_related_item")) {
?>

<li><a href="#tab_realtedProduct"

data-toggle="tab"><?php
    echo _uc($_e['Related Products']);
?></a></li>
...
<li><a href="#tab_images" data-toggle="tab"><?php
echo _uc($_e['Images']);
?></a></li>

<li><a href="#tab_setting" data-toggle="tab"><?php
echo _uc($_e['Setting']);
?></a></li>
...
echo _uc($_e['Product Basic Information']);
?></h2>
...
HTML;
    if ($product->editPid != "") {
        ...
        $qry               = "SELECT * FROM `proudct_detail_spb` WHERE `prodet_id` = ? ";
        $editData          = $dbF->getRow($qry,array($product->editPid));
    ...
    for ($i = 0; $i < sizeof($lang); $i++) {
        ...
        if ($isEdit) {
            ...
            $eName = $eTag = $eShortDesc = $eLongDesc = $eEditFeature = $eEditFeatureP = $specification = $size_chart = $category = "";
        }
...
<div class="panel-body">';
        ...
        $form_fields[] = array(
            'label' => _uc('Select Category'),
            'name' => "$product->prefix_productBasicInformation[category]",
            'format' => "<select name='$product->prefix_productBasicInformation[category]' class='categ form-control'>
                    <option selected disabled>Select Category</option>
					<option value='software'>Software</option>
					<option value='packages'>Packages</option>
					<option value='log books'>Log Books</option>
					<option value='courses'>Courses</option>
                    <option value='services'>Services</option>
                </select><script>$('.categ').val('$category').change();</script>"
        );
        $form_fields[] = array(
...
<h2 class="tab_heading"><?php
echo _uc($_e['Product Price']);
?></h2>



<div>   <?php
$product->createPricingViewSystem();
?>
...
<h2 class="tab_heading" id="pro_cat"><?php
echo _uc($_e['Product Category']);
?></h2>
...
<h2 class="tab_heading"><?php
echo _uc($_e['Product Sizes']);
?> </h2>



<div class=""><?php
$product->createListOfScales();
?></div>
...
<div class=""><?php
$product->createListOfCustomSize();
?></div>
...
<h2 class="tab_heading"><?php
echo _uc($_e['Product Sizes Weight']);
?></h2>

<small><?php
echo _n($_e['Enter Exact name that use in product Size name, IF Size Name not match Product Weight will not work in shipping and It will use product default weight']);
?>  </small>

<div class=""><?php
$product->createListOfScalesWeight();
?></div>
...
<h2 class="tab_heading"><?php
    echo _uc($_e['Free Gift Product']);
?></h2>

<small class=""><strong><?php
    echo _uc($_e['When this product exist in cart then which free gift add in cart, please select']);
?></strong></small><br><br>
...
<h2 class="tab_heading"><?php
echo _uc($_e['Get This Feature Look Products']);
?></h2>
...
<br> <?php
echo _n($_e["Product Main Image Size : 450x300, Detail Image Size:450x300"]);
?>
...
<?php
echo _uc($_e['Drop images here to upload.']);
?>
...
(<?php
echo _fc($_e['they will only be visible to you']);
?>)
...
// );
if ($isEdit) {
    // $linkDiscount  = '<a href="-product_spb?page=pDiscountForm&pId=' . $product->editPid . '" class="btn btn-success" target="_blank">' . _uc($_e['Manage Discount']) . '</a>';
    // $form_fields[] = array(
...
/*echo _uc($_e['Minimum Quantity Allow']); */
?> </label>
...
/*echo $checked; */
?> data-id="minQty" class="QtyAllow"><?php
/*echo _uc($_e['Use Config Setting']); */
?></label>
...
<label class="col-sm-2 control-label"><?php
/*echo _uc($_e['Maximum Quantity Allow']); */
?> </label>
...
<label><input type="checkbox" <?php
/*echo $checked; */
?> data-id="maxQty" class="QtyAllow"><?php
/*echo _uc($_e['Use Config Setting']); */
?></label>
...
<label><input type="checkbox" <?php
echo $checked;
?> data-id="selectType" class="QtyAllow"><?php
echo _uc($_e['Use Config Setting']);
?></label>
...
?> data-old="<?php
echo $selectType;
?>" id="selectType" class="form-control" data-name="<?php
echo $product->prefix_setting;
?>[selectType]" name="<?php
echo $product->prefix_setting;
?>[selectType]">
...
onsubmit="return submitProduct();"><?php
echo _u($_e['SUBMIT']);
?></button>
...
<?php
if ($isEdit) {
    $sqll      = "SELECT * FROM `product_category_spb` WHERE `procat_prodet_id` = ? ";
    $ress      = $dbF->getRow($sqll,array($editId));
...
url: 'product_management_spb/product_ajax.php?page=productEditImageDel',
...
jAlertifyAlert("<?php
echo _js($_e['Image Not Delete Please Try Again']);
?>");
...
url: 'product_management_spb/product_ajax.php?page=pImageAltUpdate',
...
url: 'product_management_spb/product_ajax.php?page=sortProductImage',
...
jAlertifyAlert("<?php
echo _js($_e['There is an error, Please Refresh Page and Try Again']);
?>");
