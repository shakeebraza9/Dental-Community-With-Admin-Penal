...
`service_link`, `service_heading`, `category`, `service_shrtDesc`,`service_image`,`service_position`,`service_email`,`service_date`,`service_type`,`publish`)
VALUES (?,?,?,?,?,?,?,?,?,?)";

$array   = array($link,$heading,$category,$short_desc,$service_image,$service_position,$service_email,$service_date,$service_type,$publish);
...
$array   = array($link, $heading, $category, $short_desc,
...
@$category = unserialize($data['category']);
...
//CATEGORY
$category_name_db       = $category;
            $category_names_str     = $this->functions->ibms_setting('category');
            $category_names_array   = explode(',', $category_names_str);
            $select_option         = '<option disabled="disabled" selected="selected" value="">-- Select</option>';
            foreach ($category_names_array as $category_name) {
 $select_option .= '<option value="' . trim($category_name) . '" 
                '.trim($category_name == ($category_name_db) ? ' selected="" ' : "").' 
...
<div class="col-sm-10  col-md-9">
<textarea name="service_shrtDesc['.$lang[$i].']" id="service_shrtDesc" maxlength="500" class="'.$classEditor.' form-control" placeholder="'. _uc($_e['Description']) .'">'.@$service_shrtDesc[$lang[$i]].'</textarea>
</div>
</div>';
...
<input type="text" id="service_link"  value="'.@$data['service_link'].'" name="service_link" class="form-control" placeholder="'. _uc('Service Link') .'">
...
// $sql_cat = "SELECT category FROM service";
// $data_cat =  $this->dbF->getRows($sql_cat);

$category_names_str      =  $this->functions->ibms_setting('category');
// $category_names_str      =  $data_cat;
$category_names_array   = explode(',', $category_names_str);
$category_names         = '<option disabled="disabled" selected="selected" value="">-- Select</option>';
foreach ($category_names_array as $category_name) {
$category_names .= '<option value="' .trim($category_name) . '" >' .$category_name . '</option>' ;
}

echo'
<div class="form-group">
<label class="col-sm-2 col-md-3  control-label">'. _uc($_e['Select Category']) .':</label>

<div class="col-sm-10  col-md-9">
<div class="select-group">
<select name="category" value="'.$category.'" class="form-control" placeholder="" required="" >
' . $select_option . '
</select>      
</div>
</div>
