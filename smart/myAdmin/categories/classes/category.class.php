...
class WebMenu extends object_class{
    ...
    public function newMenuAdd(){
        ...
        if(isset($_POST['submit'])){
            ...
            $name_array['English'] = $_POST['heading']['English'];
            $name_array['German'] = '';
            $name_array['French'] = '';
            // $name_array['English'] = '';

            $name = serialize($name_array);

            $desc_array['English'] = $_POST['short_desc']['English'];
            $desc_array['German'] = '';
            $desc_array['French'] = '';
            // $desc_array['English'] = '';

            $desc = serialize($desc_array);
            $underMenu_new = 'ss'.$underMenu;
...
$lastId_new = 'ss'.$lastId;
$sql_new  =   "INSERT INTO `categories`(`id`,`name`,`short_desc`, `link`,`icon`, `under`,`type` {$link_for_sql_one} ) VALUES (?,?,?,?,?,?,? {$link_for_sql_two} )";
$array_new    =     array($lastId_new,$name,$desc,$link,$icon,$underMenu_new,$menuType);
$con->setRow($sql_new,$array_new,false);
$pageLink_new = '/'.$this->db->pCategory.$lastId_new;
$ref_id_new = $this->db->pCategory.$lastId_new;
$array1_new   = array($pageLink_new,$ref_id_new,$lastId_new,$name,1);      
$con->setRow($sql1,$array1_new,false);
...
$sql_new  =   "INSERT INTO `categories`(`id`,`name`,`short_desc`, `link`,`icon`, `under`,`type` {$link_for_sql_one} ) VALUES (?,?,?,?,?,?,? {$link_for_sql_two} )";
$array_new    =     array($lastId_new,$name,$desc,$link,$icon,$underMenu_new,$menuType);
$conIntra->setRow($sql_new,$array_new,false);
$pageLink_new = '/'.$this->db->pCategory.$lastId_new;
$ref_id_new = $this->db->pCategory.$lastId_new;
$array1_new   = array($pageLink_new,$ref_id_new,$lastId_new,$name,1);      
$conIntra->setRow($sql1,$array1_new,false);
                ...
                $array1   = array($pageLink,$ref_id,$lastId,$heading,1);
...
{$link_for_sql_one}
WHERE id = '$lastId_new'";              
$array_new    =     array($name,$desc,$link,$icon,$underMenu_new,$menuType);
// custom working for related links menu
( $link_for_sql_one != '' ) ? array_push($array_new,$link_for_value) : '';
$con->setRow($sql_new,$array_new,false);
...
$sql_new1  =   "UPDATE `categories` SET
`name` = ?,
`short_desc`  = ?,
`link` = ?,
`icon` = ?,
...
{$link_for_sql_one}
WHERE id = '$lastId_new'";              
$array_new    =     array($name,$desc,$link,$icon,$underMenu_new1,$menuType);
// custom working for related links menu
( $link_for_sql_one != '' ) ? array_push($array_new,$link_for_value) : '';
$conIntra->setRow($sql_new1,$array_new,false);
                ...
                // $array1   = array($pageLink,$lastId,$heading);
