...
class order_ajax extends object_class
{
    ...
    public function getOrderProductJson()
    {
        ...
        $sql = "SELECT `proudct_detail_spb`.`prodet_id`, `proudct_detail_spb`.`prodet_name`,`product_price_spb`.`propri_intShipping`
            FROM `proudct_detail_spb` join `product_price_spb` on
                `proudct_detail_spb`.`prodet_id`=`product_price_spb`.`propri_prodet_id`
                where `product_price_spb`.`propri_cur_id`='$countryId'
                ORDER BY `proudct_detail_spb`.`prodet_id` ASC";
    ...
    public function getOrderProductStoreJson()
    {
        ...
        $sql = "SELECT * FROM `product_inventory` WHERE `qty_product_id` = ?  AND `qty_product_scale` =  ?  AND `qty_product_color` = ? ";
        $product = $this->dbF->getRows($sql,array($pId,$scleId,$colorId));
        $JSON = '[';
    ...
    public function finalPriceShipping()
    {
        ...
        $sql = "SELECT * FROM `shipping` WHERE hash = ? ";
        $data = $this->dbF->getRow($sql,array($hash));
        $array = array();
        if ($this->dbF->rowCount > 0) {
            $array['find'] = "1"; // Found
    ...
    public function delOrder()
    {
        try {
            ...
            $oldData = $this->dbF->getRows($sql,array($id));
...
$sql2in = "DELETE FROM invoices WHERE order_id='$id'";
$this->dbF->setRow($sql2i, false);
$this->dbF->setRow($sql2in, false);
...
$this->dbF->error_submit($e);
    ...
    public function quick_invoice_update()
    {
        if (!empty($_POST["orderid"]) && isset($_POST["invoice"])) {
            ...
            $id = $order_id;
            $invoice_id = htmlspecialchars($_POST["invoice"]);
            $inv = $invoice_id;

            $sql = "SELECT * FROM `order_invoice` WHERE order_invoice_pk = ? ";
            $dataTrans = $this->dbF->getRow($sql,array($id));
            ...
            if (($inv == '0' || $inv == '3' || $inv == '6')) {
                $sql = "SELECT inTransaction,rsvNo,rsvNo_done FROM `order_invoice` WHERE order_invoice_pk = ? AND inTransaction!=''";
                $dataTrans = $this->dbF->getRow($sql,array($id));
            ...
            if ($this->dbF->rowCount > 0) {
                echo "1";
                {
                    ...
                    $data_info = $this->dbF->getRow($sql,array($id));
...
intval($order_id);
intval($product_id);
htmlspecialchars($schedule_date);
htmlspecialchars($schedule_slot);
htmlspecialchars($sevice_type);
            ...
            <script type="text/javascript">
                $(document).ready(function(){
                    ...
                    $("#submitTechnicalForm").on("click", function(){
                        ...
                        $.ajax({
                            url: "order_spb/order_ajax.php?page=submitTechnicalForm",
                            type: "post",
                            data: technical_form
                        }).done(function(res){
                            console.log(res);
...
htmlspecialchars($home_setting);
htmlspecialchars($time_on);
htmlspecialchars($time_off);
htmlspecialchars($daily_hours);
htmlspecialchars($month_days);
htmlspecialchars($clockTime_set);
htmlspecialchars($pwr_mode);
htmlspecialchars($hvac_fan);
htmlspecialchars($frag_intense);
htmlspecialchars($comments);
htmlspecialchars($technician_name);
htmlspecialchars($machine);
htmlspecialchars($serial_no);
htmlspecialchars($system_location);
htmlspecialchars($level_before);
htmlspecialchars($level_after);
htmlspecialchars($signature);
htmlspecialchars($complete);
...
$admin_email = $this->functions->send_mail($adminMail,"","","serviceComplete",$cname,$mailArray, false);
    ...
    public function cancelAgreement($orderId=false){

        $order_id = isset($_POST['order_id']) ? $_POST['order_id'] : $orderId;

        $sql = "SELECT `order_customer`,`order_mandate` FROM `orders` WHERE order_id= ? ";
        $data = $this->dbF->getRow($sql,array($order_id));
        $customer = $data['order_customer'];
        $mandate  = $data['order_mandate'];

        if($customer=='manual'){

            $sql = "UPDATE `orders` SET `order_status` = 'cancelled' WHERE `order_id` = ?";
            $res = $this->dbF->setRow($sql, array($order_id));
            ...
            try {
                $client->mandates()->cancel("$mandate");
                $client->customers()->remove("$customer");
        ...
        $return = '<form class="form-horizontal" id="editSchedule_form" role="form">
                    ...
                    <script>
                        $("#editSchedule_date").on("change", function(){
                            ...
                            $.ajax({
                                url: "order_spb/order_ajax.php?page=getAvailableSlots",
                                type: "post",
    ...
    function submitEditSchedule(){
        ...
        $sched_id = htmlspecialchars($_POST['sch_id']);
        $order_id = htmlspecialchars($_POST['order_id']);

        $edit_sched_date = htmlspecialchars($_POST['editSchedule_date']);
        $edit_sched_slot = htmlspecialchars($_POST['editSchedule_slot']);
    ...
    public function updateInvoice(){
     ...
     public function ndateinvoice(){

$oid = intval($_POST['orderId']);
$ndate = htmlspecialchars($_POST['ndate']);
 $oid;
 $ndate;

 if ( $_POST['ndate'] !='') {
  
            $sqlniorder = "SELECT * FROM `orders` WHERE `order_id` = ? ";
            $niorder = $this->dbF->getRow($sqlniorder,array($oid));
           // $niorder['expire_date'];
          
            $invoicePK = $this->dbF->getRow("SELECT max(due_date) FROM `invoices` WHERE `order_id` = ?  ORDER BY `due_date` ASC",array($oid));
  
        $sqlninvoice = "SELECT * FROM `invoices` WHERE `order_id` = ? AND `due_date` = ?  ORDER BY `invoices`.`due_date` ASC";
            $ninvoice = $this->dbF->getRow($sqlninvoice,array($oid,$invoicePK[0]));

      // echo  $ninvoice['due_date'];
      $price =   $ninvoice['price'];
      $invoice_status = $ninvoice['invoice_status'];
      $update_date =  $ninvoice['update_date'];
      $payment_id =  $ninvoice['payment_id'];
            
        
for($i=1 ; $i <= $ndate;  $i++) {

         //echo  "hi"."<br>";
          $due_date = strtotime($ninvoice['due_date']);
        echo  $due_date = date("Y-m-d", strtotime("+$i month", $due_date));
      
       if ($due_date == $ninvoice['due_date']) {
            
            echo  "this is a same date";
      ...
      $this->dbF->setRow($sql, array($oid,$price,$due_date,$invoice_status,$update_date), false);
     }
 ...
 $invoicePK = $this->dbF->getRow("SELECT max(due_date) FROM `invoices` WHERE `order_id` = ?  ORDER BY `due_date` ASC",array($oid));
echo $invoicePK[0];
 $sqlorder = "UPDATE `orders` SET `expire_date` = ?  WHERE `order_id` = ?";
   $this->dbF->setRow($sqlorder, array($invoicePK[0],$oid), false); 
