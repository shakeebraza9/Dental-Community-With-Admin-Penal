...
<a href="-order_spb?page=Downloadall" class='btn btn-primary btn-sm' style="font-size: 24px;">Download All</a>
...
$sql = "SELECT * FROM `invoices`  where `order_id` = '$value[order_id]' AND `due_date` <= CURDATE() AND `invoice_status`  != 'paid'  ORDER BY   `due_date` ASC";
...
if($chkk=='manual'){
	$pymt = 'Cash';
	if($val['invoice_status']=='pending' && $val['due_date']<=$dt){
...
$sqls = "SELECT `order_customer` FROM `orders` WHERE order_id='$orderId'";
