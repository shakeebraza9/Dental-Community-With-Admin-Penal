...
class menu_ajax extends object_class{
    ...
    public function deleteMenu($id=false){
        ...
        $sql = "SELECT * FROM webmenu WHERE  under = ? ";
        $data = $this->dbF->getRows($sql,array($id));
        $count = $this->dbF->rowCount;
        if(!$count){
            $sql = "DELETE FROM webmenu WHERE  id = ? ";
    ...
    public function deleteFooterMenu($id=false){
        ...
        $sql = "SELECT * FROM webfootermenu WHERE  under =  ? ";
        $data = $this->dbF->getRows($sql,array($id));
        $count = $this->dbF->rowCount;
        if(!$count){
            $sql = "DELETE FROM webfootermenu WHERE  id =  ? ";
            $this->dbF->setRow($sql,array($id));
            //echo '1delete : '.$id;
            if($first){
                echo '1';
    ...
    public function footerMenuSort(){
        $list=htmlspecialchars($_POST['menu']);
        for ($i = 0; $i < count($list); $i++) {
