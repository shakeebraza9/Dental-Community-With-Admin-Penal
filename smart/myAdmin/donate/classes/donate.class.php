...
################### NEW MODULE NOTE ##########################
//If you want to make new module like donate, just copy paste donate. and change page_type to your type
//and only change label  and hide or show any fields.

class donate extends object_class{
    public $productF;
    public $imageName;
    private $page_type = "donate";
    ...
    public function donateView(){
        ...
        // $sql  = "SELECT id, heading,publish_date,dateTime FROM donate WHERE publish = '1' AND publish_date <= '$today' ";
        $sql  = "SELECT id, heading,publish_date,dateTime FROM donate WHERE publish = '1'";
        $data =  $this->dbF->getRows($sql);
        $this->print_donate_table($data);
    }

    public function donatePending(){
        $today = date('Y-m-d');
        $page_type = $this->page_type;
        $sql  = "SELECT id, heading,publish_date,dateTime FROM donate WHERE publish = '1' AND publish_date > '$today' ";
        $data =  $this->dbF->getRows($sql);
    ...
    public function donateDraft(){
        $page_type = $this->page_type;
        $sql  = "SELECT id, heading,publish_date,dateTime FROM donate WHERE publish = '0' ";
        $data =  $this->dbF->getRows($sql);

        $this->print_donate_table($data);
    }

    private function print_donate_table($data){
            ...
            echo "<tr>
                    ...
                    <td>
                        <div class='btn-group btn-group-sm'>
                            <a data-id='$id' href='-donate?page=edit&pageId=$id' class='btn'>
    ...
    public function newNewsAdd(){
        ...
        if(isset($_POST['submit'])){
            ...
            try{
                ...
                if($file){
                    $returnImage =  $this->functions->uploadSingleImage($_FILES['image'],'donate');
    ...
    public function donateEditSubmit(){
        global $_e;
    ...
    public function donateNew(){
        ...
        echo '</div> <!-- homeP Tab End -->
                ...
                ';
                        ...
                        //Date
                        if($this->functions->developer_setting('donate_date')=='1'){
                            echo '<div class="form-group">
                                <label class="col-sm-2 col-md-3  control-label">'. _uc($_e['Date']) .'</label>
                                <div class="col-sm-10  col-md-9">
                                    <input type="text" name="date" value="'.date("Y-m-d").'" class="date form-control" placeholder="'. _uc($_e['News Date']) .'">
                                </div>
                            </div>';
                        ...
                        //Banner
                        if($this->functions->developer_setting('donate_image')=='1'){
                            echo '<div class="form-group">
                                    <label  class="col-sm-2 col-md-3  control-label">'. _uc($_e['News Image (278x278 px)']) .'</label>
                                    <div class="col-sm-10  col-md-9">
    ...
    public function donateEdit(){
        ...
        //No need to remove any thing,, go in developer setting table and set 0
        echo '<form method="post" action="-donate?page=donate" class="form-horizontal" role="form" enctype="multipart/form-data">'.
            $token.
        ...
        //Type
        // if($this->functions->developer_setting('donate_comment')=='1'){
