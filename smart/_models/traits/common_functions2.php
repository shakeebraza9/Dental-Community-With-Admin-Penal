...
trait common_function2
{
    ...
    private $uploadFiles = array(
        /*Image Type*/  "jpg","jpeg","bmp","gif","png","img","svg","webp","webm",
      ...
      /*mac windows*/   "odt", "fodt", "ods", "fods", "odp", "fodp", "odg", "fodg", "odf"
    ...
    public function deleteOldSingleImage($image){
        ...
        @unlink(__DIR__."/../../images/$image");
            ...
            function uploadResizedImages($dataUrl,$userId,$folderName){

    global $webClass, $dbF, $functions, $webClass;
        ...
        $imgNameN = rand(99,999).time().'.png';

        $imgNameN = $rand.'-'.$userId.'-'.$imgNameN;
        $imagePath = $year.$month.$imgNameN;
        $return  = $folder.$imagePath;

        file_put_contents($path.$imagePath, $data);
     ...
     */
public function uploadDocstxt($data,$folder='mixed',$file=''){
    ...
    public function uploadDocs($data,$folder='mixed',$file=''){
...
/*                    $imgName = str_replace(' ', '-', $imgName);
                ...
                $imgName    = $year.$month.$imgName;
                $return     = $folder.$imgName;
                if(copy("https://drive.google.com/uc?id=$data&export=download",$path.$imgName)){
                    return $return;
                }
                // if(move_uploaded_file($bName,$path.$imgName)){
                //     return $return;
                // }
        //     }
        // }
    ...
    public function uploadSingleImage($data,$folder='mixed',$imgName='',$fileType=''){
        ...
        if(($data["size"])>0)
        {
            ...
            //     ($data["type"] == "image/gif") || ($data["type"] == "image/jpg") || ($data["type"] == $fileType) || ('all' == $fileType))
            // {
                ...
                if ($data["error"] > 0){
                    ...
                    $realName = $data["name"];
                    if($imgName!=''){
                        $imgName = $rand.'-'.$userId.'-'.$imgName.'-'.$realName;
                    }else{
                        $imgName = $rand.'-'.$userId.'-'.$realName;
                    }
                    $imgName = sanitize_file_name($imgName);
...
/*                    $imgName = str_replace(' ', '-', $imgName);
    ...
    public function uploadSingleFile($data,$folder='files',$fileName='',$fileType='all'){
        //$fileType = 'application/pdf';
        if($this->check_file_allow($data["name"])==false) return false;
        return $this->uploadSingleImage($data,$folder,$fileName,$fileType);
    }
