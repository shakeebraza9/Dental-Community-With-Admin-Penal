...
$this->functions->includeAdminFile("product_management_spb/functions/product_function.php");
$this->functions->includeAdminFile("product_spb/classes/product.class.php");
...
public function p_code($pid)
{
//Not Proper,,, Which is default and which store user select Not define
$sql = "SELECT p_code FROM `proudct_detail` where prodet_id = ?";
...
{
$sql = "SELECT * FROM `product_image_spb` WHERE product_id = ? AND alt = ? ORDER BY sort ASC ";
$data = $this->dbF->getRow($sql,array($id,$alt));
...
public function productSpecialImageStock($id, $alt)
{
$sql = "SELECT * FROM `product_image` WHERE product_id = ? AND alt = ? ORDER BY sort ASC ";
$data = $this->dbF->getRow($sql,array($id,$alt));
if ($this->dbF->rowCount > 0) {
$imag = $data['image'];
} else {
//get first Image
$imag = $this->productFirstImageStock($id);
}
return $imag;
}


public function productFirstImageStock($pId)
{
$data = $this->productAllImageStock($pId, '1', true);
if (!empty($data)) {
$imag = $data['image'];
return $imag;
}
...
public function productAllImageStock($id, $limitP = false, $OnlyFirstImage = false)
{
$limit = '';
if ($limitP != false) {
...
$sql = "SELECT * FROM `product_image` WHERE product_id = ? ORDER BY sort ASC $limit";
...
$sql = "SELECT * FROM `product_image_spb` WHERE product_id = ? ORDER BY sort ASC $limit";
...
{
$today = date('m/d/Y');
$sql = "SELECT * FROM `product_setting_spb`
WHERE
...
public function pBox($pId, $wishList = false, $view = true, $latest = false)
    {
        global $_e;
        ...
        //         return false;
        //     }
            // $box = $this->webClass->getBox('box6');
            // $outOfStock = '<img src="'.$box['image'].'" style="position: absolute;top: 0;width: 100%;max-width: 230px;display: block;left: 0px;right: 0px;margin: 0 auto;">';
        //     $outOfStock = '<div class="mytag">
        //                         <div class="mytag_name">Sold</div>
        //                         <!-- mytag_name -->
        //                         <div class="mytag_bottom"></div>
        //                         <!-- mytag_bottom -->
        //                     </div><!-- mytag -->';
        // }
 ...
 // $this->dbF->prnt($productImage);
        ...
        $d = $this->dbF->getRow("SELECT `setting_val` FROM `product_setting_spb` WHERE `p_id`= ? AND `setting_name`= ? ",array($pId,'ldesc'));
        $ldesc = translateFromSerialize($d['setting_val']);
        ...
        // echo base64_decode($_GET['coupon']);
           // $sql1 = "SELECT * FROM product_coupon_spb WHERE pCoupon_pk = ? ";
      $sql1 = "SELECT * FROM product_coupon_spb WHERE pCoupon_pk = ? and (pCoupon_products ='' OR pCoupon_products IN ($pId))";
        ...
        $colorDiv = "";
            if ($hasColor) {
               ...
               $colorDiv = "<div class='container_detail_RightR_color_heading'>
                                   <dl id='sample_$pId' class='dropdown'> <dt><a> <span>" . $_e['Select a color'] . "</span></a></dt><dd>
                            <ul> " . $colorDiv .  "    </ul>
                              </dd></dl>" . "</div>";
            ...
            if ($this->functions->developer_setting('product_customSize') == '1') {
                $sql = "SELECT * FROM product_size_custom WHERE `pId` = ? ";
                $customData = $this->dbF->getRows($sql,array($pId));
            ...
            if(!empty($scaleDiv)){
                $scaleDiv = "<div class='container_detail_RightR_color_heading'>
                                   <dl class='sample_select_$pId dropdown_select'> <dt><a> <span>" . $_e['Select Scale'] . "</span></a></dt><dd>
                            <ul> " . $scaleDiv .  "    </ul>
                              </dd></dl>" . "</div>";
            }
        ...
        if ($newPrice != $pPrice && $pPriceActual != '') {
            ...
            $oldPriceDiv = $pPriceActual.' '. $currencySymbol;
        ...
        if ($isSale) {
            ...
            //Sale
            $saleDivDis = "
                <div class='col2_box_tag'>
                    <div class='col2_box_tag_txt'>" . $_e['SALE'] . "</div>
                    <!-- col2_box_tag_txt close -->
                    <div class='col2_box_tag_bottom'></div>
                    <!-- col2_box_tag_bottom close -->
                </div>";
        ...
        if($latest){
            $latestDiv = "
                <div class='col2_box_tag'>
                    <div class='col2_box_tag_txt'>" . $_e['New'] . "</div>
                    <!-- col2_box_tag_txt close -->
                    <div class='col2_box_tag_bottom'></div>
                    <!-- col2_box_tag_bottom close -->
                </div>";
        ...
        // $staple_product_category_spb1 = $this->productF->check_product_in_staple_cat($pId);
        // if($staple_product_category_spb1){
        //     $staple_product_category_spb = "<div class='three_for_2_icon bundle_cat_icon'><img src='".WEB_URL."/webImages/bundle_icon.png' /></div>";
        // }else{
        //     $staple_product_category_spb = "";
        // }
...
HTML;
            ...
            <div class='col1_box' data-id='$pId' data-sr='enter bottom and scale up 80% over 1.5s' >
                $wishListRemove
                <input type='hidden' class='hidden' value='$discountFormat' id='discountFormat_$pId'>
                <input type='hidden' class='hidden' value='$discountP' id='discount_$pId'>
                <input type='hidden' class='hidden' value='$newPrice' id='discountPrice_$pId'>
                ...
                <a href='$link'>
                    <div class='col1_box_img'> <img src='$productImage' alt='$pName'>
                        $saleDivDis
                        $latestDiv 
                        $outOfStock
                    </div>
                    ...
                    <div class='col1_box_txt'>
                        <h3>$pName</h3>";
                 $chk = $this->dbF->getRow("SELECT `setting_val` FROM `product_setting_spb` WHERE `setting_name`='matchproduct' AND `p_id`=$pId");
                         $chk = $chk[0];
                         if(empty($chk)){
                             $chk=0;
                         }   
                         if($chk != 1){      
                 $temp .= "<h2>$newPriceDiv</h2>
                        <h4>$oldPriceDiv</h4>";
                    }
                 $temp .= "<div class='col1_box_btn'> <span>$showDetailsT</span> </div>
                        <!-- col1_box_btn close -->
                    </div>
                    <!-- col1_box_txt close -->
                </a>
...
<a href='$link'>
  ...
  <div class='pgrid_prize_mob'>
              ...
              if($data['validity'] > 1){$permonthChk = 'Per Month';}
              if($pCoupon_format == 'price'){$vatText = 'Included VAT';}else{$vatText = 'Plus VAT';}
            ...
            <h1 class="pricingTable-firstTable_table__header">$pName</h1>
                                    <p class="pricingTable-firstTable_table__pricing"><span>£$pPriceActual</span></p>
                                    <p><span class="light">Plus VAT monthly $permonthChk</span></p>

                                    <button class="pricingTable-firstTable_table__getstart"><a href='javascript:;' onclick='selectProduct($pId);'>Get Started</a></button>
             <img src="webImages2/aiom.svg" class="priving-img" alt="">
                                    $ldesc
                                    <button class="pricingTable-firstTable_table__getstart w-demo">Watch Demo</button>
                                </li>
            <!-- col4_top_box end-->
HTML;
            ...
            $productImage = $this->functions->resizeImage($productBigImage, '175', '330', false);
            $temp = <<<HTML
             <div class="col8_left_box box-m-1">
                                    <div class="col8_left_box_img boxes-image-in">
                                        <img src="images/$productBigImage" alt="">
                                    </div>
                                    <!-- col8_left_box_img close -->
                                    <div class="col8_left_box_txt textex-te">
                                        <h4>$pName</h4>
                                       
                                        <span class="price">£$pPriceActual</span> <span>Plus VAT $permonthChk</span>
                                        <div>
                                        <a href='javascript:;' class="book-btn book-btn-form" onclick='selectProduct($pId);'>Book Now</a>
                                        
                                             
                                            <a href='javascript:;' class="book-btn book-btn-form2" data-name='$pName' style="margin-left: 10px;">Get Started</a>
                                            
                                            <br>
                                            <span style="margin-right: 10px;">(Up to 15 delegates)</span>
                                             <span>(More than 15 delegates)</span>
                                        </div>
            ...
            $temp = <<<HTML
                     <div class="col8_left_box box-m-1">
                                    ...
                                    <div class="col8_left_box_txt textex-te">
                                        ...
                                        <span class="price">£$pPriceActual</span> <span>Plus VAT $permonthChk</span>
                                        <div>
                                        <a href='javascript:;'  class="book-btn book-btn-form" onclick='selectProduct($pId);'>Get Started</a>
                                        </div>
                                    </div>
                                    <!-- col8_left_box_txt close -->
...
$cId = $val;
$LIKE .= " `product_category_spb`.`procat_cat_id` LIKE '%$cId%' OR";
}
$LIKE = trim($LIKE, "OR");
...
$newField
FROM `product_category_spb`
JOIN
`proudct_detail_spb` as detail
  on `product_category_spb`.`procat_prodet_id` = `detail`.`prodet_id`
...
switch ($sortBy) {
case "lowPrice":
$sql = " , (SELECT propri_price FROM product_price_spb as sr
...
$newField
FROM
   `proudct_detail_spb` as detail join `product_setting_spb` as setting
...
$sql = "SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
FROM
   `proudct_detail_spb` join `product_setting_spb`
    on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
    WHERE
        `product_setting_spb`.`setting_name`='publicAccess'
        AND `product_setting_spb`.`setting_val`='1'
        AND `proudct_detail_spb`.feature = '1'
        AND `proudct_detail_spb`.`product_update`='1' ORDER BY sort ASC";
...
FROM
   `proudct_detail_spb` join `product_setting_spb`
    ...
    WHERE
        `product_setting_spb`.`setting_name`='publicAccess'
        AND `product_setting_spb`.`setting_val`='1'
        AND `proudct_detail_spb`.feature = '2'
        AND `proudct_detail_spb`.`product_update`='1' ORDER BY sort ASC";
...
FROM
   `proudct_detail_spb` join `product_setting_spb`
    ...
    WHERE
        `product_setting_spb`.`setting_name`='publicAccess'
        AND `product_setting_spb`.`setting_val`='1'
        AND `proudct_detail_spb`.`product_update`='1' ORDER BY prodet_id DESC";
...
FROM
   `proudct_detail_spb` join `product_setting_spb`
    ...
    WHERE
        `product_setting_spb`.`setting_name`='publicAccess'
        AND `product_setting_spb`.`setting_val`='1'
        AND `proudct_detail_spb`.`product_update`='1' ORDER BY sale DESC";
$sql .= " LIMIT $limitFrom,$limitT0";
$productIds = $this->dbF->getRows($sql);
//var_dump($productIds);
...
if (empty($relatedIds)) {
$sql = "SELECT * FROM (
SELECT `proudct_detail_spb`.*, `product_setting_spb`.`setting_val`
FROM
   `proudct_detail_spb` join `product_setting_spb`
    on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
    WHERE
        `product_setting_spb`.`setting_name`='publicAccess'
        AND `product_setting_spb`.`setting_val`='1'
        AND `proudct_detail_spb`.`product_update`='1'
...
$sql = "SELECT * FROM product_category_spb WHERE procat_prodet_id = '$pId'";
...
$LIKE .= " `product_category_spb`.`procat_cat_id` LIKE '%$catIds%' OR";
}
$LIKE = trim($LIKE, "OR");
...
//Find Product That in this category
$sql = "SELECT `prodet_id` FROM `product_category_spb`
JOIN
`proudct_detail_spb`
  on `product_category_spb`.`procat_prodet_id` = `proudct_detail_spb`.`prodet_id`
    WHERE $LIKE
      GROUP BY `proudct_detail_spb`.`prodet_id` ORDER BY RAND()";
$products = '';
...
//Get Pid Category
//return all related categorys product...
$sql = "SELECT * FROM product_category_spb WHERE procat_prodet_id != '$pId' ORDER BY RAND()";
$categoryData = $this->dbF->getRow($sql);
if (!$this->dbF->rowCount) {
return false;
}
...
//return all related categorys product...
$data = array();
$sql_ = "SELECT * FROM `product_setting_spb` WHERE setting_name = 'related' AND p_id = ?";
...
$ids=rtrim($ids,', ');
// var_dump($ids);
$sql = "SELECT * FROM `proudct_detail_spb` WHERE prodet_id IN ($ids) ORDER BY RAND() LIMIT $limit";
...
LEFT OUTER JOIN `proudct_detail_spb` p ON p.prodet_id = b.product_id
...
$price = "AND `prodet_id` in (SELECT `propri_prodet_id` FROM product_price_spb WHERE propri_price BETWEEN $pMin AND $pMax)";
}
...
$category = "AND `prodet_id` in (SELECT `procat_prodet_id` FROM product_category_spb WHERE `procat_cat_id` LIKE '%$catId%')";
}
}
...
AND `product_setting_spb`.`setting_val`='1'
AND `proudct_detail_spb`.`product_update`='1'
AND `proudct_detail_spb`.`prodet_id` = '$pId'";
$data = $this->dbF->getRow($sql);
return $data;
}
...
############ Product Categories ##########
$pro_cat    = $this->productF->product_category_spb($pId);
...
public function proProDesc($id = false){


$sql1 = "SELECT `prodet_shortDesc` FROM `proudct_detail` WHERE `prodet_id` = ?";
$res1 = $this->dbF->getRow($sql1, array($id));

$cat_name = translateFromSerialize($res1['prodet_shortDesc']);
...
public function getCoupon_hash($id='remove',$from=false,$to=false,$hash=false,$hash2=false){
...
$_SESSION['webUser']['coupon_id'] = $id;
$_SESSION['webUser']['coupon_from'] = $from;
$_SESSION['webUser']['coupon_to'] = $to;
$_SESSION['webUser']['coupon_hash'] = $hash;
$_SESSION['webUser']['coupon_hash2'] = $hash2;
} else if (isset($_SESSION['webUser']['coupon_id'])) {

$id = $_SESSION['webUser']['coupon_id'];
$from = $_SESSION['webUser']['coupon_from'];
$to = $_SESSION['webUser']['coupon_to'];
$hash = $_SESSION['webUser']['coupon_hash'];
$hash2 = $_SESSION['webUser']['coupon_hash2'];
} else {
...
if (isset($id) && $id == 'remove') {

$_SESSION['webUser']['coupon_id'] = '';
$_SESSION['webUser']['coupon_from'] = '';
$_SESSION['webUser']['coupon_to'] = '';
$_SESSION['webUser']['coupon_hash'] = '';
$_SESSION['webUser']['coupon_hash2'] = '';
$coupon = '';
$id = '';
$hash = '';
$from = '';
...
?,?,?,?,?,?,?
 ...
 htmlspecialchars($_POST['sender_name']);
 htmlspecialchars($_POST['sender_phone']);
 htmlspecialchars($_POST['sender_email']);
 htmlspecialchars($_POST['sender_address']);
 htmlspecialchars($_POST['sender_city']);
 htmlspecialchars($_POST['sender_country']);
 htmlspecialchars($_POST['sender_post']);
 htmlspecialchars($_POST['receiver_name']);
 htmlspecialchars($_POST['receiver_phone']);
 htmlspecialchars($_POST['receiver_email']);
 htmlspecialchars($_POST['receiver_address']);
 htmlspecialchars($_POST['receiver_city']);
 htmlspecialchars($_POST['receiver_country']);
 htmlspecialchars($_POST['receiver_post']);
$array = array(
$invoiceId, $userId,
...
{
$sql = "SELECT MIN(propri_price) as min,MAX(propri_price) as max FROM `product_price_spb`";
$data = $this->dbF->getRow($sql);
...
FROM `proudct_detail_spb` detail INNER JOIN `product_setting_spb` setting
ON detail.prodet_id = setting.p_id
WHERE
...
$sql = "SELECT detail.prodet_id,detail.slug, detail.prodet_name, setting.p_id, setting.setting_val
          FROM `proudct_detail_spb` detail INNER JOIN `product_setting_spb` setting ON detail.prodet_id = setting.p_id
...
//             SELECT `prodet_id` FROM `product_category_spb` 
//             JOIN `proudct_detail_spb` as detail on `product_category_spb`.`procat_prodet_id` = `detail`.`prodet_id` 
...
//                 `proudct_detail_spb` join `product_setting_spb`
//                 on `proudct_detail_spb`.`prodet_id` = `product_setting_spb`.`p_id`
//                 WHERE `product_setting_spb`.`setting_name`='publicAccess'
//                 AND `product_setting_spb`.`setting_val`='1'
//                 AND `proudct_detail_spb`.`product_update`='1' ORDER BY sale DESC
//                 LIMIT 182000000
...
INNER JOIN product_category_spb ON product_category_spb.procat_prodet_id = pd.prodet_id
WHERE {$cat_like_sql}
...
{
$sql  = " SELECT `slug` FROM `proudct_detail_spb` WHERE `prodet_id` = ? AND product_update = '1' ";
...
// var_dump($_POST);
if (isset($_POST) && !empty($_POST) && !empty($_POST['email']) && !empty($_POST['pname'])) {

if (!$this->functions->getFormToken('WebOrderReady')&&$_POST['api']!="api") {
return false;
}
...
$this->dbF->setRow($sql, array($orderId,$full_name,$mobile,$address,$city,$email), false);
...
if(isset($_GET['redirect_flow_id']) && !empty($_GET['redirect_flow_id'])){
    ...
    $orderId = $data[0];
    $data = $this->dbF->getRow("SELECT `invoice_pk`,`price` FROM `invoices` WHERE order_id = '$orderId' AND due_date <= CURRENT_DATE() LIMIT 1");
