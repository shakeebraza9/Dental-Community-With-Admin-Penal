...
$icon           = empty($_POST['icon'])      ? "0"    : $_POST['icon'];
$underMenu      = empty($_POST['underMenu']) ? "0"    : $_POST['underMenu'];
...
$image_alt      = empty($_POST['image_alt']) ? "0"    : $_POST['image_alt'];
...
htmlspecialchars($short_desc);
htmlspecialchars($underMenu);
htmlspecialchars($menuType);
htmlspecialchars($link_for_value);
...
$sql        =   "SELECT * FROM webmenu where id =  ? ";
...
$is_top_most_menu = ( isset($data['under']) && $data['under'] == '0' && $data['type'] != 'related links' || isset($_GET['type']) && $_GET['type'] == 'main' || $_GET['type'] == 'report_menu') ? TRUE : FALSE;
...
// echo this->functions->developer_setting('main_menu_icon');
// exit;
...
// var_dump(data_unCompress($data['short_desc']));
@$heading = unserialize($data['name']);
@$shortDesc = unserialize($data['short_desc']);
...
$short_desc_css = ( $is_top_most_menu == FALSE ) ? '' : '';
...
$id     =   $_GET['menuId'];
$sql    =   "SELECT * FROM webfootermenu where id = ? ";
$data   =   $this->dbF->getRow($sql,array($id));
if($this->dbF->rowCount==0){
