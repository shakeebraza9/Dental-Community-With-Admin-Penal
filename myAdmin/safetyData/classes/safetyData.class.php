...
class safetyData extends object_class{
    public $productF;
    ...
    public function safetyDataSort(){
        ...
        $sql ="SELECT * FROM `safetyData` WHERE publish = '1' ORDER BY sort ASC";
    ...
    public function safetyDataView(){
        $sql  = "SELECT * FROM safetyData WHERE publish='1' ORDER BY ID DESC";
        $data =  $this->dbF->getRows($sql);
        $this->safetyDataPrint($data);
    }

    public function safetyDataDraft(){
        $sql  = "SELECT * FROM safetyData WHERE publish='0' ORDER BY ID DESC";
        $data =  $this->dbF->getRows($sql);
        $this->safetyDataPrint($data);
    }

    public function safetyDataPrint($data){
        global $_e;
        $class = 'tableIBMS';
        $heading = false;
            ...
            echo "<td>$i</td>
                    ...
                    <td>
                        <div class='btn-group btn-group-sm'>
                            <a data-id='$id' href='-safetyData?page=edit&fileId=$id' class='btn'>
                                <i class='glyphicon glyphicon-edit'></i>
                            </a>
                            <a data-id='$id' onclick='deleteSafetyData(this);' class='btn'>
    ...
    public function newsafetyDataAdd(){
        global $_e;
        ...
        $savepath = WEB_URL . "/uploads/files/resources/smartDoc".str_replace($search,'', $title).".el";
             ...
             fclose($fp);
              ...
              else
              {
                ...
                $sql = "INSERT INTO `safetyData`(`title`, `file`, `extra_file`, `ytcode`, `psp`, `category`,`sub_category`,`sub_sub_category`,`assignto`,`publish`)
                                    VALUES (?,?,?,?,?,?,?,?,?,?)";
    ...
    public function safetyDataEditSubmit(){
        global $_e;
        if(isset($_POST['submit'])){
    ...
    public function safetyDataNew(){
        global $_e;
        $this->safetyDataEdit(true);
    }
...
$temp = "";
$cats  = $this->functions->ibms_setting('safetyDataCategory');
$cats  = explode(",",$cats);
foreach($cats as $cat) {
...
return $temp;
    ...
    public function safetyDataEdit($new=false){
        global $_e;
         $checked = ""; 
         $dspy0='';
         $dspy1 = '';
        if($new){
            ...
            $id = $_GET['fileId'];
            $sql = "SELECT * FROM safetyData where id = '$id' ";
...
fclose($handle);
    ...
    $dspy0='style="display:block"';
    ...
    else
      {
        ...
        echo '<form method="post" action="-safetyData?page=safetyData" class="form-horizontal" role="form" enctype="multipart/form-data">'.
            $token.
            '
