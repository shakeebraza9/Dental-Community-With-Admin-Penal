...
class issues_ajax extends object_class{
...
public function deleteIssues(){
    ...
    try{
        ...
        $sql3="SELECT image FROM issues WHERE id=?";
        ...
        $sql2="DELETE FROM issues WHERE id= ?";
       $this->dbF->setRow($sql2,array($id));
        if($this->dbF->rowCount) echo '1';
        else echo '0';

        $this->db->commit();
        $this->functions->setlog(($_e['Delete']),($_e['Issues']),$this->dbF->rowLastId,($_e['Issues Delete Successfully']));
