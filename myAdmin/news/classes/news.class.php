...
class news extends object_class{
    ...
    public function newsView(){
        ...
        // $sql  = "SELECT id, heading,publish_date,dateTime FROM news WHERE publish = '1' AND publish_date <= '$today' ";
        $sql  = "SELECT id, heading,publish_date,dateTime FROM news WHERE publish = '1'";
    ...
    public function newsDraft(){
        $page_type = $this->page_type;
        $sql  = "SELECT id, heading,publish_date,dateTime FROM news WHERE publish = '0' ";
        $data =  $this->dbF->getRows($sql);
                ...
                if($this->dbF->rowCount>0){
                   $heading = translateFromSerialize($heading);
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        $uid = $value['acc_id'];
                         $sqlcron    =   "INSERT INTO `email_queue` (`email_to`, `subject`, `message`) VALUES (?,?,?)";
                         $this->dbF->setRow($sqlcron,array($email,$heading,$content));
