...
class product_function extends object_class{
 ...
 public function getProductFullName($pid,$scaleId,$colorId){
    ...
    public function getProductFullNameSTOCK($pid,$scaleId,$colorId){
        $pName = $this->getProductNameSTOCK($pid);
        if($pName==false){
            return false;
        }
        if( $scaleId != '0'){
            $sName=  $this->getScaleNameSTOCK($scaleId);
        }else{
            $sName = "";
        }
        if($colorId != '0'){

            $cName = $this->getColorNameSTOCK($colorId);
            $cName ="<span style='background:#$cName;padding: 4px 10px;color: #fff;font-size: 11px;'>$cName</span>";
        }else{
            $cName = "";
        }
     ...
     * @return string
     */
    public function getProductNameSTOCK($pid){
        if($pid == '0'){
            return "";
        }
        $defaultLang= $this->functions->AdminDefaultLanguage();
        ...
        $data   =   $this->dbF->getRow($sql);
        if($this->dbF->rowCount>0){
          return  $name   =   translateFromSerialize($data['prodet_name']);
            // return  $name[$defaultLang];
        }
        return false;
    }
   ...
   /**
     ...
     * @return string
     */
    public function getScaleNameSTOCK($id){
        if($id == '0'){
            return "";
        }
     ...
     * @return string
     */
    public function getColorNameSTOCK($id){
        if($id == '0'){
            return "";
        }
    ...
    public function stockProductInventory(){
       ...
       // $sql = "SELECT * FROM  `product_inventory_detail` ORDER BY `qty_store_id`,`qty_pk` ASC";
        $data =$this->dbF->getRows($sql);
        return $data;
    }
     public function stockProductInventory_detail(){
        // $sql = "SELECT * FROM  `product_inventory` ORDER BY `qty_store_id`,`qty_pk` ASC";
        $sql = "SELECT * FROM  `product_inventory_detail` ORDER BY `qty_store_id`,`qty_pk` ASC";
        $data =$this->dbF->getRows($sql);
        return $data;
    }
 public function stockProductInventory_min($min_stock,$id){


       $sql = "SELECT * FROM  `product_inventory` WHERE qty_item <= '$min_stock' AND qty_product_id = '$id' ORDER BY `qty_store_id`,`qty_pk` ASC";
        $data =$this->dbF->getRow($sql);
        return $data;
    }
...
$inventoryData = $this->dbF->getRow($sqli);
$invQty =   $inventoryData[0];
if($invQty > 0 && $invQty >= $qty){
// echo "string0000";


// $sqlz = "SELECT * FROM `removeQTY` WHERE hash=?  AND order =?";
// $this->dbF->getRow($sqlz,array($hash,$order_invoice_id));
// if ($this->dbF->rowCount>0) {


// $sqlu    =   "UPDATE `removeQTY` SET `qty` = qty+'$qty' WHERE `hash` = '$hash'";
// $dbF->setRow($sqlu);
...
// $deduct = $qty - $invQty;
// $sqlz = "SELECT * FROM `removeQTY` WHERE hash=?";
// $dbF->getRow($sqlz,array($hash));
// if ($dbF->rowCount>0) {
...
return $qtyInve;
    ...
    public function SKU($pId){
     ...
     public function AjaxDelScript2($class,$name){
        global $_e;
        echo "<script>
        function AjaxDelScript2(ths){
            btn=$(ths);
            if(secure_delete()){
    ...
    public function AjaxUpdateScript2_d($class,$name){
        global $_e;
        echo "<script>
        function AjaxUpdateScript2_d(ths){
            btn=$(ths);
            if(secure_update()){
                btn.addClass('disabled');
                btn.children('.trash').hide();
                ...
                }).done(function(data)
                {
                   if(data=='1'){
                     setTimeout(function(){
                        ...
                        btn.children('.trash').show();
                        btn.children('.waiting').hide();
                        jAlertifyAlert('".ucwords($name)." ". _js($_e['Update Fail Please Try Again.']) ."');
                    }
                   else{
                        btn.removeClass('disabled');
                        btn.children('.trash').show();
    ...
    public function productSQL2($columnName='*',$AllWithNewBlank=true){
        $all ='';
        if($AllWithNewBlank===true){
            $all = "WHERE `product_update` = '1' AND prodet_id IN (SELECT DISTINCT(`qty_product_id`) FROM `product_inventory`)";
        }else{
            $all    =   $AllWithNewBlank;
        }
    ...
    public function productJSON2(){
        $product=$this->productSQL2('`prodet_id`,`prodet_name`',true);
        $defaultLang= $this->functions->AdminDefaultLanguage();
        $JSON='[';
    ...
    public function AjaxUpdateScript2($class,$name){
        global $_e;
        echo "<script>
        function AjaxUpdateScript2(ths){
            btn=$(ths);
            if(secure_update()){
                btn.addClass('disabled');
                btn.children('.trash').hide();
...
//     }
    ...
    public function scaleSQL2($id,$columnName='*',$groupByName = true){
        if($groupByName){
            $groupByName = "GROUP BY `prosiz_name`";
        }else{
            $groupByName = "";
        }
         $sql="SELECT ".$columnName." FROM `product_size` WHERE `prosiz_prodet_id` = '$id' $groupByName";
        return $this->dbF->getRow($sql);
    }
 ...
 public function colorSQL2($pId,$columnName='*'){
        $sql="SELECT ".$columnName." FROM `product_color` WHERE `proclr_prodet_id` = '$pId' GROUP BY `proclr_name` ORDER BY propri_id ASC";
        return $this->dbF->getRow($sql);
    }
...
//get sale offer
        ...
        // $this->dbF->prnt($this->productSaleData);
