...
var columnDefsSevenColumns = [
        ...
        if (url == "setting/setting_ajax.php?page=active_history") {
           uid = $(this).attr("data-uid");
          url = url + "&uniq="+uniq+ "&id="+ uid;
        }
        else{
          url = url + "&uniq="+uniq;
        }   
...
header: true,
// exportOptions: {
// columns: [':contains("ORDER NUMBER")',':contains("EXTRA INFO")',':contains("ASSIGN TO BOX/GROUP:")']
// }
}
...
$('.dTable_ajax_email').each(function(){
        url = $(this).attr("data-href");
        uniq = $(this).attr("data-uniq");
        <?php # add uniq as $_GET param on the current link ?>
        url = url + "&uniq="+ uniq;
