...
class testimonial extends object_class{
    ...
    public function newTestimonialAdd(){
        ...
        if(isset($_POST['submit'])){
            ...
            $testimonial_category      = empty($_POST['testimonial_category'])          ? ""    : ($_POST['testimonial_category']);

             $video_link  = empty($_POST['video_link'])          ? ""    : ($_POST['video_link']);
            ...
            $video_link = serialize($this->functions->removeWebUrlFromLink($video_link));        
            try{
                $this->db->beginTransaction();

                $sql      =   "INSERT INTO `testimonial`(
                                    `testimonial_link`, `testimonial_heading`, `testimonial_shrtDesc`,`testimonial_image`,`testimonial_position`,`testimonial_email`,`testimonial_date`,`publish`,`testimonial_category`,`video_link`)
                                    VALUES (?,?,?,?,?,?,?,?,?,?)";

                $array   = array($link,$heading,$short_desc,$testimonial_image,$testimonial_position,$testimonial_email,$testimonial_date,$publish,$testimonial_category,$video_link);
        ...
        if(isset($_POST['submit'])){
            ...
            try{
                ...
                $array   = array($link, $heading, $short_desc,
                                ...
                                $publish,$testimonial_category,$video_link);
                $this->dbF->setRow($sql,$array,false);
                $this->functions->setting_fieldsSet($lastId,'testimonial',false);
        ...
        for ($i = 0; $i < sizeof($lang); $i++) {
            ...
            if($lay1_Status>0){
                ...
                echo '<div class="form-group">
                        ...
                        <div class="col-sm-10  col-md-9">
                            <select name="testimonial_category"  class="testimonial_category_'.$i.' form-control">
                  ...
                  echo '<div class="form-group">
                            <label class="col-sm-2 col-md-3  control-label">'. _uc($_e['Video Id']) .' </label>
                            <div class="col-sm-10  col-md-9">
                                <input type="text"  name="video_link['.$lang[$i].']" value="'.@$video_link[$lang[$i]].'" class="testimonial_position_'.$i.' form-control" placeholder="">
                            </div>        
                        </div>';       

                    //<input type="text"  name="testimonial_category['.$lang[$i].']" value="'.@$testimonial_category[$lang[$i]].'" class="testimonial_category_'.$i.' form-control" placeholder="">;
