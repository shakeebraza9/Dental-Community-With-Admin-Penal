...
<?php 
    ...
    // include_once('./api/vendor/stripe/index.php');
    
    global $productClass, $functions,  $sFunction;
    // $sFunction->checking();
    // exit;
    $cusId = 'cus_NxWH2kiQnOVZvy';
    $sFunction->inactiveUser($cusId);
    ...
    exit();
    // parameters
    $userData = $productClass->isCustomerExist();
    var_dump($userData);
  
    function orderSumit(){
        
        global $productClass, $functions,  $sFunction;
        if (!$functions->getFormToken('WebOrderReady')) {
                return false;
    }else{
        $successUrl = WEB_URL.'/test.php';
        $failedUrl = WEB_URL.'/cancel.php';
        $customerId = '';
        $userData = array(
            'email' => 'basit@gmail.com' ,
            'userName' => 'Jawwad Rafique',
            'phone' => '0311-84571811',
            'address' => 'L-112 Sec 33/C Korangi',
            'city' => 'Karachi',
            'proName' => 'ABC',
            'amount' => 12500,
            'quantity' => 1,
        ...
        // $paymentId = 'pm_1NAX2aARYcQcurTJVUAz6pWq';
        // $cutomerId = 'cus_NwPstj3z7q4UdV';
        // $amount = 25205;
        // $sFunction->payRecurringPayment($paymentId, $cutomerId, $amount);
        $sFunction->recurringPayment($successUrl, $failedUrl, $customerId, $userData);
    }
    ...
    if(isset($_GET['status'])){
        ...
        if(isset($_GET['session_id'])){
            ...
            $dbF->prnt($session);
        }else if(isset($_GET['pId']) && isset($_GET['sId'])){
            echo  $_GET['pId'] ;
            echo  $_GET['sId'] ;
        }
                        ...
                        <form method="POST">
                            ...
                            <div class="inputs_flex">
                                <div class="input-feild booking-form">
                                    <input type="text" placeholder="Practice Name" name="pName" >
                                </div>
                                
                                <div class="input-feild booking-form">
                                    <input type="text" placeholder="Practice Number" name="mobile" >
                                </div>
                            ...
                            <div class="inputs_flex">
                                <div class="input-feild booking-form">
                                    <input type="text" placeholder="Practice Address" name="address" >
                                </div>
                                
                                <div class="input-feild booking-form">
                                    <input type="text" placeholder="Practice City Address" name="city" >
                                </div>
                            </div>

                            <div class="input-field additional">
                                <div class="input-feild booking-form">
                                    <input type="email" placeholder="Practice Email Address" name="email" >
                                </div>
                            </div>
