...
if($t == 'Compliance-Templates'){$t='Compliance Templates';
}elseif($t == 'HR-Management'){$t='HR Management';}else{$t='Compliance Templates';}

if (@$_GET['id'] !='') {
    $token = $functions->setFormToken('editComplianceTemplate',false);

} else {
    $token = $functions->setFormToken('addComplianceTemplate',false);
}
...
// }
$option = $functions->complianceTemplateCategory(@$user);
...
<div class="event_details" id="myform">
    <h3>Upload Compliance Template</h3>
    <div class="form_side">
        <form method="post" action="resources?category=Compliance-Templates" enctype="multipart/form-data">
            <?php echo $token ?>
