...
public function getEventData($id, $columns){
    $sql =  "Select $columns from `userevent` where id = '$id'";
    $data = $this->dbF->getRows($sql);
    return $data[0] ;
}
...
{
$user = $_SESSION['currentUser'];
$sql = "SELECT COUNT(`category`) FROM `eventmanagement` WHERE `assignto` IN ('all','$user') AND `publish` = '1' AND `due_date`<=(SELECT CURDATE()) AND id NOT IN(SELECT title_id FROM `userevent` JOIN `eventmanagement` ON `eventmanagement`.`id` = `userevent`.`title_id` WHERE `approved` IN ('1','0','-1') AND `user`='$user') AND id NOT IN (SELECT recurrence FROM eventmanagement) AND `type` !='updates'";
$sql2 = "SELECT COUNT(`category`) FROM `userevent` JOIN `eventmanagement` ON `eventmanagement`.`id` = `userevent`.`title_id` WHERE `approved`='-1' AND `user`='$user' AND `userevent`.`assignto` < '1' AND `userevent`.`due_date`<=(SELECT CURDATE()) AND `type` !='updates'";
$data = $this->dbF->getRow($sql);
$data2 = $this->dbF->getRow($sql2);
return $data[0] + $data2[0] + 0;
}
...
{
$user = intval($_SESSION['currentUser']);
$sql = "SELECT * FROM `eventmanagement` WHERE `assignto` IN ('all','$user') AND `publish` = '1' AND `due_date`<=(SELECT CURDATE()) AND id NOT IN(SELECT title_id FROM `userevent` JOIN `eventmanagement` ON `eventmanagement`.`id` = `userevent`.`title_id` WHERE `approved` IN ('1','0','-1') AND `user`='$user') AND id NOT IN (SELECT recurrence FROM eventmanagement) AND `type` !='updates' ORDER BY `title`";
$sql2 = "SELECT `userevent`.`id`,`userevent`.`due_date`,`title`,`category`,`type`,`userevent`.`file` as `userfile`,`approved`,`userevent`.`assignto`,`userevent`.`dateTime` FROM `userevent` JOIN `eventmanagement` ON `eventmanagement`.`id` = `userevent`.`title_id` WHERE `approved`='-1' AND `user`='$user' AND `userevent`.`assignto` < '1' AND `userevent`.`due_date`<=(SELECT CURDATE()) AND `type` !='updates' ORDER BY `title`";
$data = $this->dbF->getRows($sql);
$data2 = $this->dbF->getRows($sql2);
$mysql = $this->setli($data);
$mysql .= $this->setli($data2, 'editevent');
...
$sql1 = "SELECT  COUNT(*) FROM `myevents` WHERE (`assignto` IN ('-1.$d[0]','$user')  OR  `user`='$user' ) AND (
...
$sql = "SELECT  * FROM `myevents` WHERE (`assignto` IN ('-1.$d[0]','$user')  OR  `user`='$user' ) AND (
(`status`!='deleted' AND `status`!='complete')
OR  (      
...
{
echo "<script> console.log('My event Submit') </scrtipt>";
if (isset($_POST['submit'])) {
...
if (strpos($assignto, '-1') !== false) {
    ...
    if($this->getEventData($edit_id, 'assignto') != $assignto){
...
{
$sql = "SELECT title FROM `eventmanagement` WHERE `assignto` IN ('all','$user') AND `due_date` <= (SELECT CURDATE()) AND `id` NOT IN (SELECT `title_id` FROM `userevent` WHERE `user`='$user') AND `publish` = '1' AND `id` NOT IN (SELECT `recurrence` FROM `eventmanagement`) AND `type` !='updates'";
$sql2 = "SELECT title FROM `userevent` JOIN `eventmanagement` ON `eventmanagement`.`id` = `userevent`.`title_id` WHERE `approved`='-1' AND `user`='$user' AND `userevent`.`due_date` <= (SELECT CURDATE()) AND `type` !='updates'";
$data = $this->dbF->getRows($sql);
$data2 = $this->dbF->getRows($sql2);
$mysql = array_merge($data, $data2);
$mysql = array_unique($mysql, SORT_REGULAR);
...
$sql.= 
        ...
        `desc` as description,
        `type` as eventType
        FROM `myevents` WHERE (`assignto` IN ('-1.$d[0]','$user') OR  `user`='$user')  AND (
(`status`!='deleted' AND `status`!='complete')
OR  (      
...
$practice_logo = $this->uploadSingleImage($_FILES['practice_logo'],$user, 'practice profiles');
}

if ($img2) {
$this->deleteOldSingleImage($old_team_image);
$team_image = $this->uploadSingleImage($_FILES['team_image'],$user,'practice profiles');
}
...
$li = "";
foreach ($data as $key => $value) {
$li .= '<div class="rowdiv ' . (($value['type'] == 'mandatory') ? 'redborder ' : 'blueborder').'" onclick="' . $page . '(this.id)" id="' . $value['id'] . '" data-type="' . (($value['type'] == 'mandatory') ? 'redborder' : 'blueborder') . '" data-toggle="tooltip"  >
        ...
        <div class="colunmdiv" style ="max-width: 140px;">
            <h6>Type</h6>
            <h5>'.$value['type'].'</h5>
        </div> <div class="colunmdiv" style ="max-width: 90px;">
            <h6>Due Date</h6>
