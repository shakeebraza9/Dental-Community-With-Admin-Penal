...
$this->STRIPE_PUBLISHABLE_KEY = 'pk_test_51N8fNsARYcQcurTJlba1Qc53f8oqMvpWHHTSiLYPTsY0t9kGxEjif1cRsANIIq8dvXgZmOQBbkUWf8kLDM0NS5Q800thbmfjZc';
...
$sql = "SELECT * FROM `store_name` ORDER BY store_pk ASC ";
$data = $this->dbF->getRow($sql);
$_SESSION['webUser']['store_Id'] = $data['p_code'];
}
...
<a href='$link'>
  ...
  <div class='pgrid_prize_mob'>
            ...
            $PPricee = str_replace('USA', '', $pPrice);
            $priceee="$".$PPricee;
            $type  = $data['validity'] > 1 ? 'recurring_payment' : 'oneoff_payment';

            $temp = <<<HTML
                
                <div class="single_pricing">
                    <div class="price">
                         <h1>$priceee a month</h1>
                    </div>
                    <div class="p_detail">
                        <p class="pro_name" >$pName</p>
                        <p>$ldesc</p>
                    </div>
                    <button class="pricing_btn" data-id='$pId' data-ref='$type'  >Buy Now</button>
                </div>
HTML;
...
public function isCustomerExist($email = ""){
    ...
    if($isLogin > 0){
       ...
       $sql = "SELECT `acc_cus_id` FROM `accounts_user` WHERE `acc_email` = '$email_temp'";
       $tempData = $this->dbF->getRow($sql);
       if($this->dbF->rowCount > 0){
           $data['customer'] = $tempData['acc_cus_id'];
           $data['user'] = 1;
...
$array = array($orderId,$full_name,$mobile,$address,$city,$email);
$this->dbF->setRow($sql,$array , false);
