...
// echo div when task perform fail
 ...
 // Error show
    ...
    */
    ...
    public function setRow($query,$arr=null,$tryCatch=true,$update=false){
        ...
        try{
            ...
            if($array_with_key){
                foreach($arr as $key=>$val){
                    $i++;
                    $sth->bindValue($i, htmlspecialchars($val), PDO::PARAM_STR);
                }
            }
                else{
                if(is_array($arr)){
                for ($i=0;$i<sizeof($arr);$i++) {
                    $index=$i+1;
                    $sth->bindValue($index, htmlspecialchars($arr[$i]), PDO::PARAM_STR);
                }
                }else{
                  for($i=0;$i<$arr;$i++){
                  $index=$i+1;     
                  $sth->bindValue($index, htmlspecialchars($arr[$i]));
                  }
              }
          }
 ...
 *
 */
     public function setRowMultiTable($query,$arry,$query2=false,$valFormat="",$arry2=array(),$tryCatch=true){
