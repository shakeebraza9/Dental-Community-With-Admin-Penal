...
if($obtain_perc >= $pass_percent){
    ...
    $result = "You have PASSED the CPD please <a href='printResult.php?assignid=$aassign_id&user=$user' target='_blank'>click here</a> to download your certificate";
    ...
    <div class="standard">
        ...
        if( $dataFeedback == TRUE  && $dataFeedback['feedback'] =="" ){
         echo   "<script>$(document).ready(function(){feedbackRating('$assign_id','$test_id');});</script>";
          ?>
        <h2 style="text-align: center; width: 30%;margin: 6px auto;" ><a href="javascript:;" onclick="feedbackRating('<?php echo $assign_id ?>','<?php echo $test_id ?>');" class="btn edit_btn" >FeedBack</a></h2>
<?php   } ?>
    </div>
    <!-- standard -->
</div>
