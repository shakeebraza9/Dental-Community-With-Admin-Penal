...
global $productClass;
if($seo['title']==''){
$seo['title'] = $dbF->hardWords('Product List',false);
}
...
// $msg  = "";
$chk2  = $functions->editStockLocation();
if($chk2){
$msg = "Location Added / Edit Successfully";
}

$chk1  = $functions->editmin_stockPK();
if($chk1){
$msg = "Min Stock Added / Edit Successfully";
}
$chk10  = $functions->editexpiryDate();
if($chk10){
$msg = "Expiry Date Added / Edit Successfully";
}




$chk10  = $functions->addDeductQty();
if($chk10){
$msg = "Product Add / Deduct Successfully";
}
...
<!-- right_side_top close -->
 <?php if($msg!=''){ ?>
	...
	<li><a active href="#tabs-1">Current Products</a></li>
	<?php 
	...
	{ ?>
	<li><a href="#tabs-2">Add Existing Stock Form</a></li>
	<?php } ?>
	</ul>   
	<div id="tabs-1">
...
<th><?php echo _u('Stock storage location'); ?> </th>
<th><?php echo _u('Minimum Quantity level'); ?></th>
...
<tbody>
<?php 
$sqls = "SELECT * FROM `product_inventory`";
...
$sqls  .= " ORDER BY `product_inventory`.`qty_pk` DESC";
...
<td>
<?php
// echo $value['qty_product_color'];
echo   $productClass->getProductFullNameWeb( $value['qty_product_id'], $value['qty_product_scale'], $value['qty_product_color']);
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Edit Expiry Date"  class="ablue2" id="' . $value['qty_pk'] . '" onclick="editexpiryDate(this.id)"> ('.$value['expiryDate'].')</a>';
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Add Expiry Date" class="ablue2" id="' . $value['qty_pk'] . '" onclick="editexpiryDate(this.id)"> Add Expiry Date</a>';
}
}
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Add / Deduct Quantity"  class="ablue2" id="' . $value['qty_pk'] . '" onclick="addDeductQty(this.id)">'.$value['qty_item'].'</a>';
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Edit location"  class="ablue2" id="' . $value['qty_pk'] . '" onclick="editStockLocation(this.id)">'.$value['location'].'</a>';
}
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Add location" class="ablue2" id="' . $value['qty_pk'] . '" onclick="editStockLocation(this.id)">Add Location</a>';
}}
?>
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Edit Min Stock Quantity"  class="ablue2" id="' . $value['qty_pk'] . '" onclick="editStockmin_stock(this.id)">'.$value['min_stock'].'</a>';
...
{ 
     ...
     echo '<a data-toggle="tooltip" title="Add product to basket" id="' . $value['qty_product_id'].":".$value['qty_product_scale'].":".$value['qty_product_color'] . '" class="add-btn" onclick="addTobasket(this.id)"><i class="fas fa-cart-plus"></i></a>';
     echo "<button class='del-btn' title='Delete product' type='button' id='$value[qty_pk]' onclick='dltproductlist(this.id)' style='margin-left: 10px;'><i class='far fa-trash-alt'></i></button>";   
}
...
<div id="tabs-2">
<form class="profile" method="post" action="stockView" enctype="multipart/form-data">
...
<input type="text" name="receipt_date" autocomplete="off" value="<?php echo $data['receipt_date'] ?>" id="purchaseReceiptDATE">
</div>
...
<input type="hidden" name="receipt_date" value="">
<input type="hidden" name="receipt_prf" value="">
<input type="hidden" name="receipt_ponumber" value="">
<input type="hidden" name="receipt_receiver" value="">
<input type="hidden" name="receipt_supplier" value="">
<!-- <div class="form-group col-12 col-sm-6">
<label>Receiver :</label> -->
<!-- <?php  //$functions->receiverDroupdownOptions(); ?> -->
<!-- </div> -->
...
$userSa = intval($_SESSION['currentUser']);
 ...
 ?>
  <script>
    function dltproductlist(id){
        var result = confirm("Are you sure you want to delete?");
        if (result) {
            $.ajax({
                type: 'post',
                data: {id:id},
                url: 'ajax_call.php?page=dltproductlist',                
            }).done(function(data) {
                if (data == '1') {
...
var pScale = $(".receipt_product_scale").val();
var pColor = $(".receipt_product_color").val();
...
// var trpid = "p_"+pid+"-"+scaleVal+"-"+colorVal+"-"+store;
var trpid = "p_"+pid+"-"+scaleVal+"-"+colorVal;
